  ...
  implements _Server, BaseWithHttpOptions
{
  ...
  closeAllConnections(): void {
    ...
    throw new ERR_METHOD_NOT_IMPLEMENTED('closeAllConnections');
  }

  closeIdleConnections(): void {
    // It doesn't make sense to support this method.
    throw new ERR_METHOD_NOT_IMPLEMENTED('closeIdleConnections');
  }

  setTimeout(
    msecs?: number | undefined | ((socket: Socket) => void),
    callback?: ((socket: Socket) => void) | undefined
  ): this {
  ...
  async #onRequest(
    ...
    // Request is `socket` in Node.js
    this.emit('connection', this, request);
    return new Response('Hello, World!');
  }

  listen(...args: unknown[]): this {
    const [options, callback] = _normalizeArgs(args);
    if (typeof options.port !== 'number' || portMapper.has(options.port)) {
      throw new Error(`Port ${options.port} is already in use`);
    }
    portMapper.set(options.port, { fetch: this.#onRequest.bind(this) });
    callback?.();
    return this;
  }
}
  ...
  implements _ServerResponse
{
  ...
  writeHead(
    ...
    let headers: Record<string, { name: string; value: string | string[] }>;
    if (this[kOutHeaders]) {
      ...
      if (Array.isArray(obj)) {
        ...
        for (let n = 0; n < obj.length; n += 2) {
          k = obj[n + 0] as string;
          if (k) this.appendHeader(k, obj[n + 1] as string);
        }
      } else if (obj) {
        ...
        for (let i = 0; i < keys.length; i++) {
          k = keys[i] as string;
          if (k) this.setHeader(k, obj[k] as string);
        }
      }
