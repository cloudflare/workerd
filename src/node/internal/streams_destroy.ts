/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-redundant-type-constituents, @typescript-eslint/no-unsafe-call */
...
function isRequest(stream: any): stream is OutgoingMessage {
...
export function destroyer(
  ...
  if (isServerRequest(stream)) {
    ...
    stream.abort();
  } else if ('req' in stream && isRequest(stream.req)) {
  ...
  if (!stream.destroyed) {
    // @ts-expect-error TS7053 Symbols are not available on types yet.
