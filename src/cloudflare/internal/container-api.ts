...
class ContainerEntrypointWithMethods<
  Env = unknown,
> extends entrypoints.DurableObject {
  defaultPort?: number;
  ...
  async start(
    ...
    if (this.container.running && !options.waitForReady) {
    ...
    while (attempt < options.retries) {
      ...
      if (
        ...
        (attempt === 0 || retryableError(lastError))
      ) {
      ...
      try {
        ...
        if (this.container.running) {
          ...
          await startupMonitor.catch((err: unknown) => {
            ...
            if (retryableError(err) && err instanceof Error) {
              lastError = err;
            } else {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const targetPort =
      this.defaultPort ?? (portFromUrl ? parseInt(portFromUrl) : undefined);
...
export default { ContainerEntrypoint: ContainerEntrypointWithMethods };
