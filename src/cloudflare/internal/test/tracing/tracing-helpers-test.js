...
export class OverlappingRequestsObject extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (requestName === 'stale') {
      ...
      assert.strictEqual(spans.invocation.isTraced, false);
...
export const activeAndInvocationSpans = {
  async test(ctrl, env, ctx) {
    ...
    assert.strictEqual(publicTracing.getInvocationSpan().isTraced, true);
    assert.strictEqual(ctx.tracing.getInvocationSpan().isTraced, true);
