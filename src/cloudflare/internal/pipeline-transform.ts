...
type PipelineRecord<T> = {
  body: T;
  readonly metadata: object;
};

export class PipelineTransformImpl<I, O> extends entrypoints.WorkerEntrypoint {
  #batch?: Batch;
  ...
  public async run(
    _records: PipelineRecord<I>[],
    _metadata: PipelineBatchMetadata
  ): Promise<PipelineRecord<O>[]> {
  ...
  private async _run(
    ...
    if (this.#batch.format === Format.JSON_STREAM) {
      ...
      const records: PipelineRecord<I>[] = data.map((obj) => {
        return { body: obj, metadata: {} };
      });
  ...
  #sendJson(records: PipelineRecord<O>[]): JsonStream {
    if (!(records instanceof Array)) {
      throw new Error('transformations must return an array of Record');
    }
    ...
    const readable = new ReadableStream<Uint8Array>({
      start(controller): void {
        for (const record of records) {
          const encoded = encoder.encode(`${JSON.stringify(record.body)}\n`);
