...
type QueryRequest = {
  queries: QuerySql[];
  bookmark?: D1SessionBookmarkOrConstraint;
};

type QueryResponse = RpcResponse<{
...
type QuerySqlResult = {
  ...
  meta: D1Meta;
  data: {
    /** Discriminator for the query result payload shape. */
    kind: 'raw';
    /** Column names for the returned rows. */
    columns: string[];
    /** Opaque row payload consumed by the D1 wrapper. */
    rows: unknown[][];
  };
};
...
class D1Database {
  ...
  /**
   * Send one or more SQL queries directly to D1 and return raw rows-and-columns results.
   *
   * Unlike `prepare().all()`, `prepare().raw()`, `prepare().first()`, or
   * `batch()`, this method does not reshape rows into objects or apply the
   * public convenience return types. It returns the lower-level D1 response
   ...
   * ```
   * await env.D1.query({
   *   queries: [{ sql: "select * from users where id=?", params: [1] }],
   *   bookmark: "<insert_bookmark>"
   * });
   * ```
   */
  async query(request: QueryRequest): Promise<QueryResponse> {
    ...
    return await fetcherQuery(this.fetcher, request);
  }
...
class D1PreparedStatement {
      ...
      const info = firstIfArray(
        d1BindingJsrpc
          ? await this.dbSession._queryOrThrow(
              { queries: [{ sql: this.statement, params: this.params }] },
              span
            )
  ...
  // TODO: Update this return type because we always includes a `results` array
  // and people are relying on that.
...
function mapQueryResult(queryResult: QuerySqlResult): D1RowsColumns {
  const { data } = queryResult;
  switch (data.kind) {
    // Keep the switch so future result kinds have an obvious extension point.
