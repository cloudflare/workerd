...
- **`Traced`**: core tracing trait in `jsg::wrappable`; built-ins include no-op impls for primitives/value types and delegating impls for wrappers/collections (`Option`, `Nullable`, `Vec`, maps/sets, `Cell`, `jsg::Member`, `jsg::Weak`, `jsg::Slot`). `jsg::Rc` and `jsg::v8::Global` are roots and are not traced.
