...
impl<'a> Local<'a, Object> {
    ...
    pub fn has(&self, lock: &mut Lock, key: &str) -> bool {
        unsafe { ffi::local_object_has_property(lock.get_isolate(), &self.handle, key) }
    }
    ...
    pub fn get(&self, lock: &mut Lock, key: &str) -> Option<Local<'a, Value>> {
        ...
        unsafe {
            let maybe_local = ffi::local_object_get_property(lock.get_isolate(), &self.handle, key);
            let opt_local: Option<ffi::Local> = maybe_local.into();
            opt_local.map(|local| Local::from_ffi(lock.get_isolate(), local))
        }
    }
}
...
impl<T> Global<T> {
    ...
    pub unsafe fn make_weak(
        ...
        isolate: *mut ffi::Isolate,
        data: *mut c_void,
        callback: fn(*mut ffi::Isolate, data: usize) -> (),
    ) {
        unsafe {
...
impl ToLocalValue for u8 {
    fn to_local<'a>(&self, lock: &mut Lock) -> Local<'a, Value> {
        unsafe {
            Local::from_ffi(
                lock.get_isolate(),
                ffi::local_new_number(lock.get_isolate(), f64::from(*self)),
            )
        }
    }
}
...
impl ToLocalValue for &str {
    fn to_local<'a>(&self, lock: &mut Lock) -> Local<'a, Value> {
        unsafe {
            Local::from_ffi(
                lock.get_isolate(),
                ffi::local_new_string(lock.get_isolate(), self),
            )
        }
    }
}
