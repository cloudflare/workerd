...
pub fn create_resource_constructor<R: Resource>(
    ...
    unsafe {
        v8::ffi::create_resource_template(lock.get_isolate(), &get_resource_descriptor::<R>())
            .into()
    }
}
...
pub fn unwrap_resource<'a, R: Resource>(
    ...
    let ptr = unsafe { v8::ffi::unwrap_resource(lock.get_isolate(), value.into_ffi()) as *mut R };
    unsafe { &mut *ptr }
}
...
impl Lock {
    ...
    fn get_realm(&mut self) -> &mut Realm {
        unsafe { &mut *crate::ffi::realm_from_isolate(self.get_isolate()) }
    }
}

/// Reference-counted pointer to a resource exposed to JavaScript.
///
/// `Ref<T>` wraps an `Rc<UnsafeCell<T>>` to allow multiple references to a resource while
/// permitting interior mutability. When wrapping a resource for V8, the `Ref` is leaked via
/// `into_raw()` to create a stable pointer, then reconstructed via `from_raw()` during cleanup.
...
impl Realm {
    ...
    pub fn add_resource(&mut self, resource: *mut ResourceState) {
...
impl Drop for Realm {
    fn drop(&mut self) {
        ...
        for resource_ptr in &self.resources {
            unsafe {
                let resource_state = &mut **resource_ptr;
                ...
                {
                    // Call the drop function to properly clean up the leaked Ref
                    drop_fn(resource_state.isolate, resource_state.this);
                    resource_state.this = std::ptr::null_mut();
                }
            }
        }
