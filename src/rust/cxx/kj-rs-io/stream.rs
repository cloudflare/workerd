...
async fn read_min(io: &Socket, buf: &mut [MaybeUninit<u8>], min_bytes: usize) -> Result<usize> {
    let min_bytes = min_bytes.min(buf.len());
    let mut total = 0;
    while total < buf.len() {
        match io.try_read(&mut buf[total..]) {
            ...
            Err(e) if would_block(&e) => io.readable().await.map_err(op("poll()"))?,
...
async fn write_all(io: &Socket, buf: &[u8]) -> Result<()> {
    let mut written = 0;
    while written < buf.len() {
        match io.try_write(&buf[written..]) {
            Ok(0) => return Err(op("write()")(wrote_zero_bytes())),
            Ok(n) => written += n,
            Err(e) if would_block(&e) => io.writable().await.map_err(op("poll()"))?,
            Err(e) if interrupted(&e) => {}
...
async fn write_all_pieces(io: &Socket, pieces: &crate::ffi::KjPieces) -> Result<()> {
    ...
    while !bufs.is_empty() {
        match io.try_write_vectored(bufs) {
