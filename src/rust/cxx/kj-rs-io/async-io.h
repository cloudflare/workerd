...
class TokioAsyncIoStream final: public kj::AsyncIoStream {
 public:
  ...
  // Gives up the Rust stream to a caller that will drive the socket itself (kj-hyper's
  // `TokioStream::into_socket`). The wrapper holds nothing afterwards, and any further call on
  // it throws. Whether the socket can actually be taken out of the Rust stream (no I/O in flight)
  // is decided on the Rust side, which hands the stream back if not.
