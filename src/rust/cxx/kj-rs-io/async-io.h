...
class TokioAsyncIoStream final: public kj::AsyncIoStream {
 public:
  ...
  // AsyncIoStream. abortRead() is shutdown(SHUT_RD), as in KJ's AsyncStreamFd: on unix a pending
  // read observes EOF (kj-http's WebSocket abort and CONNECT error paths rely on this). On
  // Windows the shutdown happens but ends no read: tokio issues recv() only after the poller
  // reports readability, and the AFD poller reports no event for a local receive shutdown, so a
  // read ends there only when the peer sends or closes. A limitation of this backend on Windows
  // to weigh before enabling it there by default.
