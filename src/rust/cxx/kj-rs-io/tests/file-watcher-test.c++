...
// Bound for "the change fires". File events are near-immediate on both backends, so a passing
// run never waits this long; the bound only has to be safe under sanitizers on a loaded CI
// runner, where 5 s was not (TSAN: 8 s to deliver a delete + recreate).
constexpr kj::Duration FIRE_TIMEOUT = 30 * kj::SECONDS;
