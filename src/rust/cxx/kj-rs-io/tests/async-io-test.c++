...
KJ_TEST("multi-piece write(): more empty pieces than IOV_MAX before the data still delivers it") {
  ...
  // than that used to make the kernel write zero bytes, which the write loop reported as
  // "connection closed" while the real data sat further down the array.
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();
  auto pair = makeTcpPair(io);
...
KJ_TEST("every stream operation starts inside the call: an unawaited write goes out") {
  // kj-http queues header writes without awaiting them; KJ's native streams issue the syscall
  // inside write(). The socket path, the fd path, and reads must all behave that way.
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();
  auto pair = makeTcpPair(io);
  ...
  auto output =
      io.getLowLevelProvider().wrapOutputFd(fds[1], kj::LowLevelAsyncIoProvider::TAKE_OWNERSHIP);
  auto unawaitedFd = output->write("pipe"_kjb);  // no eagerlyEvaluate() at the call site
  kj::byte pipeBuffer[4];
  KJ_EXPECT(input->tryRead(pipeBuffer, 4, 4).wait(ws) == 4);
  KJ_EXPECT(kj::ArrayPtr<kj::byte>(pipeBuffer, 4) == "pipe"_kjb);
  unawaitedFd.wait(ws);
#endif
}
...
KJ_TEST("parseAddress applies KJ's parse-time restrictPeers check to literals") {
  // KJ (SocketAddress::parse): a literal whose *family* the filter does not allow is rejected at
  // parse time with these exact messages; IP literals of an allowed family pass parse and are
  // judged at connect()/accept(). Hostname results are never checked at parse time.
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();
  kj_rs_io::TokioNetwork network;
  ...
  auto unixBlocked = KJ_ASSERT_NONNULL(kj::runCatchingExceptions(
      [&]() { ipOnly->parseAddress("unix:/tmp/kj-rs-io-never-bound.sock", 0).wait(ws); }));
  KJ_EXPECT(unixBlocked.getDescription().contains("unix sockets blocked by restrictPeers()"),
      unixBlocked.getDescription());
#endif

  auto unixOnly = network.restrictPeers({"unix"_kj}, {});
  auto familyBlocked = KJ_ASSERT_NONNULL(
      kj::runCatchingExceptions([&]() { unixOnly->parseAddress("127.0.0.1:1", 0).wait(ws); }));
  KJ_EXPECT(familyBlocked.getDescription().contains("address family blocked by restrictPeers()"),
      familyBlocked.getDescription());

  // An allowed family parses even if the specific address will be blocked later (KJ parity).
  auto publicOnly = network.restrictPeers({"public"_kj}, {});
  auto loopback = publicOnly->parseAddress("127.0.0.1:1", 0).wait(ws);
  auto connectBlocked =
      KJ_ASSERT_NONNULL(kj::runCatchingExceptions([&]() { loopback->connect().wait(ws); }));
  KJ_EXPECT(connectBlocked.getDescription().contains("connect() blocked by restrictPeers()"),
      connectBlocked.getDescription());
}

#if !_WIN32
KJ_TEST("wrapConnectingSocketFd connects AF_UNIX sockets too") {
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();

  auto path = freshUnixSocketPath("connecting");
  auto listener = parseNow(io, kj::str("unix:", path))->listen();

  int fd;
  KJ_SYSCALL(fd = socket(AF_UNIX, SOCK_STREAM, 0));
  struct sockaddr_un sun;
  memset(&sun, 0, sizeof(sun));
  sun.sun_family = AF_UNIX;
  KJ_REQUIRE(path.size() < sizeof(sun.sun_path));
  memcpy(sun.sun_path, path.cStr(), path.size() + 1);
  kj::uint addrlen = offsetof(struct sockaddr_un, sun_path) + path.size() + 1;

  auto acceptPromise = listener->accept();
  auto client = io.getLowLevelProvider()
                    .wrapConnectingSocketFd(fd, reinterpret_cast<struct sockaddr *>(&sun), addrlen,
                        kj::LowLevelAsyncIoProvider::TAKE_OWNERSHIP)
                    .wait(ws);
  auto server = acceptPromise.wait(ws);
  ::unlink(path.cStr());

  client->write("unix"_kjb).wait(ws);
  kj::byte buffer[4];
  KJ_EXPECT(server->tryRead(buffer, 4, 4).wait(ws) == 4);
  KJ_EXPECT(kj::ArrayPtr<kj::byte>(buffer, 4) == "unix"_kjb);
}
...
KJ_TEST("a context ignores SIGPIPE: writes to a vanished peer fail, the process survives") {
  if (getenv(SIGPIPE_CHILD_ENV) != nullptr) {
    ...
    // A pipe whose read end is gone: write(2) raises SIGPIPE unless it is ignored.
    int fds[2];
    KJ_SYSCALL(pipe(fds));
    auto output =
        io.getLowLevelProvider().wrapOutputFd(fds[1], kj::LowLevelAsyncIoProvider::TAKE_OWNERSHIP);
    ...
    auto pipeException =
        KJ_ASSERT_NONNULL(kj::runCatchingExceptions([&]() { output->write("x"_kjb).wait(ws); }),
            "write to a reader-less pipe unexpectedly succeeded");
    KJ_REQUIRE(pipeException.getType() == kj::Exception::Type::DISCONNECTED,
        pipeException.getDescription());

    // A stream socket whose peer is closed: the second write (after the RST) is the SIGPIPE
    // case for sockets.
    int sv[2];
...
KJ_TEST("tryRead with minBytes == 0 returns what is available without waiting (KJ parity)") {
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();
  auto pair = makeTcpPair(io);

  kj::byte buffer[8];
  // Nothing buffered: resolves with 0 immediately rather than parking (KJ makes one read
  // attempt and returns what it got once minBytes is satisfied).
  KJ_EXPECT(pair.server->tryRead(buffer, 0, sizeof(buffer)).wait(ws) == 0);
  pair.client->write("ab"_kjb).wait(ws);
  // Give the reactor a turn so the bytes are readable, then a zero-minimum read returns them.
  io.getTimer().afterDelay(10 * kj::MILLISECONDS).wait(ws);
  KJ_EXPECT(pair.server->tryRead(buffer, 0, sizeof(buffer)).wait(ws) == 2);
}

#if __linux__
KJ_TEST("unix-abstract: addresses parse, listen, connect, and are classified as abstract peers") {
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();

  auto name = kj::str("kj-rs-io-abstract-", ::getpid());
  auto text = kj::str("unix-abstract:", name);
  auto addr = parseNow(io, text);
  KJ_EXPECT(addr->toString() == text, addr->toString());

  auto listener = addr->listen();
  auto acceptPromise = listener->acceptAuthenticated();
  auto client = parseNow(io, text)->connect().wait(ws);
  auto server = acceptPromise.wait(ws);
  client->write("abstract"_kjb).wait(ws);
  kj::byte buffer[8];
  KJ_EXPECT(server.stream->tryRead(buffer, 8, 8).wait(ws) == 8);
  KJ_EXPECT(kj::ArrayPtr<kj::byte>(buffer, 8) == "abstract"_kjb);

  // restrictPeers grammar: "unix" does not admit abstract sockets, "unix-abstract" does (KJ's
  // filter tells the forms apart by the leading NUL, so the address must reach it intact).
  kj_rs_io::TokioNetwork network;
  auto pathOnly = network.restrictPeers({"unix"_kj}, {});
  auto blocked = KJ_ASSERT_NONNULL(
      kj::runCatchingExceptions([&]() { pathOnly->parseAddress(text, 0).wait(ws); }));
  KJ_EXPECT(blocked.getDescription().contains("abstract unix sockets blocked by restrictPeers()"),
      blocked.getDescription());
  auto abstractOnly = network.restrictPeers({"unix-abstract"_kj}, {});
  abstractOnly->parseAddress(text, 0).wait(ws);  // allowed

  // An accepted peer that BOUND an abstract name is filtered as unix-abstract, not as an
  // unnamed "unix" peer: a listener that allows only "unix" drops it.
  auto listener2 = pathOnly->parseAddress(kj::str("unix:", freshUnixSocketPath("abstract-peer")), 0)
                       .wait(ws)
                       ->listen();
  auto accept2 = listener2->accept();
  {
    int fd;
    KJ_SYSCALL(fd = socket(AF_UNIX, SOCK_STREAM, 0));
    ...
    auto boundName = kj::str("kj-rs-io-abstract-client-", ::getpid());
    memcpy(bindAddr.sun_path + 1, boundName.cStr(), boundName.size());
    KJ_SYSCALL(bind(fd, reinterpret_cast<struct sockaddr *>(&bindAddr),
        offsetof(struct sockaddr_un, sun_path) + 1 + boundName.size()));
    // Connect to the path the listener bound (its getsockname), through wrapConnectingSocketFd
    // so the bound abstract name is the peer address the listener sees.
    struct sockaddr_un target;
    memset(&target, 0, sizeof(target));
    kj::uint len = sizeof(target);
    listener2->getsockname(reinterpret_cast<struct sockaddr *>(&target), &len);
    auto connectingClient =
        io.getLowLevelProvider()
            .wrapConnectingSocketFd(fd, reinterpret_cast<struct sockaddr *>(&target), len,
                kj::LowLevelAsyncIoProvider::TAKE_OWNERSHIP)
            .wait(ws);
    kj::byte probe;
    // Dropped by the listener: the client observes EOF, and the accept stays pending.
    KJ_EXPECT(boundedBy(io, connectingClient->tryRead(&probe, 1, 1), 10 * kj::SECONDS,
                  "EOF from the dropped abstract peer")
                  .wait(ws) == 0);
    KJ_EXPECT(!accept2.poll(ws));
  }
}
#endif  // __linux__
