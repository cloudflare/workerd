...
KJ_TEST("abortRead() ends a pending read with EOF (shutdown(SHUT_RD), like KJ)") {
  ...
  kj::byte buffer[8];
#if _WIN32
  // Windows: the shutdown happens, but no read observes it (async-io.h, abortRead): tokio only
  // issues recv() once the poller reports readability, and the AFD poller reports nothing for
  // a local receive shutdown. What can be checked here is that the call succeeds and the write
  // side is untouched.
