...
use cc::Build;
use r#gen::Opt;
use r#gen::error::report;
use syntax::map::Entry;
use syntax::map::UnorderedMap;

use crate::cargo::CargoEnvCfgEvaluator;
pub use crate::cfg::CFG;
pub use crate::cfg::Cfg;
use crate::deps::Crate;
use crate::deps::HeaderDir;
...
fn make_include_dir(prj: &Project) -> Result<PathBuf> {
    ...
    if let Some(ref original) = env::var_os("DEP_CXXBRIDGE1_HEADER") {
        ...
        out::write(shared_cxx_h, r#gen::include::HEADER.as_bytes())?;
...
fn generate_bridge(prj: &Project, build: &mut Build, rust_source_file: &Path) -> Result<()> {
    ...
    let generated = r#gen::generate_from_path(rust_source_file, &opt);
...
fn best_effort_copy_headers(src: &Path, dst: &Path, max_depth: usize) {
    // Not using r#gen::fs because we aren't reporting the errors.
    use std::fs;
