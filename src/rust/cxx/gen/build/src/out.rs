...
pub fn write(path: impl AsRef<Path>, content: &[u8]) -> Result<()> {
...
pub fn relative_symlink_file(original: impl AsRef<Path>, link: impl AsRef<Path>) -> Result<()> {
...
pub fn absolute_symlink_file(original: impl AsRef<Path>, link: impl AsRef<Path>) -> Result<()> {
    let original = original.as_ref();
    let link = link.as_ref();
...
pub fn relative_symlink_dir(original: impl AsRef<Path>, link: impl AsRef<Path>) -> Result<()> {
    let original = original.as_ref();
    let link = link.as_ref();
...
fn best_effort_relativize_symlink(original: impl AsRef<Path>, link: impl AsRef<Path>) -> PathBuf {
    ...
    if let Ok(original_canonical) = original.canonicalize()
        ...
        && let Ok(relative_canonical) = parent.join(&relative_path).canonicalize()
