...
impl<'a> OutFile<'a> {
    pub fn new(kind: OutputKind, opt: &'a Opt, types: &'a Types) -> Self {
        Self {
            header: matches!(kind, OutputKind::Header),
            opt,
            types,
    ...
    pub fn write_fmt(&self, args: Arguments) {
        ...
        assert!(Write::write_fmt(content, args).is_ok());
    }
...
impl<'a> Content<'a> {
    ...
    pub fn write_fmt(&mut self, args: Arguments) {
        assert!(Write::write_fmt(self, args).is_ok());
    }
    ...
    fn push_block_boundary(&mut self, boundary: BlockBoundary<'a>) {
        if self.blocks_pending > 0
            && self
                .blocks
                .last()
                .is_some_and(|last| boundary == last.rev())
        {
            self.blocks.pop();
        ...
        while read < self.blocks.len() {
            match self.blocks[read] {
                ...
                BlockBoundary::End(end_block) => {
                    let Some(previous) = write.checked_sub(1) else {
                        panic!("unbalanced block boundary");
                    };
                    write = previous;
