...
use syntax::trivial::TrivialReason;
use syntax::trivial::{self};
...
use crate::out::OutputKind;

pub fn r#gen(apis: &[Api], types: &Types, opt: &Opt, kind: OutputKind) -> Vec<u8> {
    let mut out_file = OutFile::new(kind, opt, types);
...
fn write_data_structures<'a>(out: &mut OutFile<'a>, apis: &'a [Api]) {
    ...
    for api in apis {
        if let Api::CxxFunction(efn) | Api::RustFunction(efn) = api
            && let Some(receiver) = &efn.sig.receiver
        {
            methods_for_type
...
fn write_struct<'a>(out: &mut OutFile<'a>, strct: &'a Struct, methods: &[&ExternFn]) {
    ...
    for method in methods {
        ...
        let indirect_call = None;
        let throws = (sig.throws || method.lang == Lang::Rust).then_some(());
...
fn write_cxx_function_shim<'a>(out: &mut OutFile<'a>, efn: &'a ExternFn) {
    ...
    if let Some(ret) = &efn.ret {
        ...
        write_indirect_return_type_space(out, ret);
        write!(out, "*return$");
    }
    ...
    write!(out, "        ");
    if indirect_return && let Some(ret) = &efn.ret {
        out.include.new = true;
        write!(out, "new (return$) ");
        write_indirect_return_type(out, ret);
        write!(out, "(");
    }
    match &efn.ret {
...
fn write_rust_function_decl_impl(
    ...
    write!(out, "{}(", link_name);
    let mut needs_comma = if let Some(receiver) = &sig.receiver {
        write!(
            out,
            "{}",
            out.types.resolve(&receiver.ty).name.to_fully_qualified(),
...
fn write_rust_function_shim_impl(
    ...
    write_rust_function_shim_decl(out, local_name, sig, indirect_call, Some(()));
    if out.header {
        writeln!(out, ";");
        return;
    }
    ...
    write!(out, "{}(", invoke);
    let mut needs_comma = if sig.receiver.is_some() {
        write!(out, "*this");
        true
    } else {
        false
...
fn write_kj_maybe(out: &mut OutFile, key: NamedImplKey) {
    ...
    writeln!(
        out,
        "static_assert(!::std::is_pointer<{}>::value, \"Maybe<T*> is not allowed in workerd-cxx. Use Maybe<T&> or Maybe<Maybe<T&>> instead.\");",
        inner
    );
    writeln!(
        out,
        "static_assert(sizeof(::kj::Maybe<{0}>) != sizeof({0}), \"kj::Maybe<T> has same size as T, which implies niche value optimization, but this is not implemented for this type\");",
        inner
    );
}
