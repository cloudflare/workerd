...
//! `kj::newPromiseAndCrossThreadFulfiller` depend on.
...
// the entire event-port business logic (`TokioPort`, the `wait`/`poll`/`wake` machinery, the
// `HiResTimer` orchestration) is *compiler-proven* to contain no hand-written unsafe. The one
// island that opts back in via `#![allow(unsafe_code)]` is `ffi.rs`: the `#[cxx::bridge]` wire plus
// the OS-syscall wrappers for precise short sleeps.
