...
mod tests {
    ...
    async fn fragmented_message_with_a_ping_between_fragments() {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn masked_compressed_frame_then_uncompressed_frame() {
        let (mut peer, ws) = server();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn invalid_compressed_data_is_a_protocol_violation() {
        assert_eq!(protocol_error(&[0xc2, 0x01, 0xff], 1 << 20).await, 1002);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn rsv2_is_a_protocol_violation() {
        assert_eq!(protocol_error(&[0xa1, 0x00], 1 << 20).await, 1002);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn decompression_is_bounded_by_max_size() {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn round_trip_across_parameters() {
        let compressible = b"workerd permessage-deflate ".repeat(8000);
        let random = pseudo_random(200_000, 7);
        for window_bits in [0, 8, 9, 12, 15] {
            for no_context_takeover in [false, true] {
                ...
                let client =
                    RustWebSocket::new(Box::new(a), Role::Client, &compression, crate::io::never());
                let server =
                    RustWebSocket::new(Box::new(b), Role::Server, &compression, crate::io::never());
                for round in 0..3 {
                    for (is_text, message) in [
                        ...
                        let (sent, received) =
                            futures::join!(client.send(is_text, message), server.receive(1 << 30));
                        sent.unwrap();
