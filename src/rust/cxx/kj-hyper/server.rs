...
impl HyperConnection {
    ...
    pub async fn serve(&self) -> Result<()> {
        ...
        // hyper's connection future must be polled inside the loop's tokio runtime context so
        // `ServeIo`'s AsyncRead/Write can (re-)register with the reactor and hyper's `TokioTimer`
        // can arm the timer wheel. The KJ loop, not a `block_on`, does the polling, so we enter
        // the context per poll.
...
impl HyperResponseSender {
    ...
    pub fn accept_websocket(&self, headers: &ffi::HttpHeaders) -> Result<Box<WsSession>> {
        ...
        let (map, mut case) = translate_websocket_response_headers(&app_headers.entries())?;
