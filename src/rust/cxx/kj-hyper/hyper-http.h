...
// A bridged (Rust-backed) kj::Promise is cold: the future does not run until the promise is
// first polled. Every kj interface implemented in this layer is hot -- the operation is in
// flight the moment the method returns, and kj-side callers (WebSocket::couple, the api
// WebSocket state machine, the hibernation manager, kj's header queue) sequence their own
// state changes on that. Start bridged futures at the seam so they observe kj's event order.
template <typename T>
...
class HyperHttpService final: public kj::HttpService {
 public:
  ...
  kj::Promise<void> request(kj::HttpMethod method,
    ...
    return hot(
        impl->request(method, url.asBytes().as<kj_rs::Rust>(), headers, requestBody, response));
  }
...
// take_kj_socket): a kj-rs-io stream gives up its tokio socket outright and the consumed,
// hollow kj stream is destroyed before this returns. Every other stream (in-memory transports,
