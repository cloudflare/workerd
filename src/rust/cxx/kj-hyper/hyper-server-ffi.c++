...
class HyperResponseBodyStream final: public kj::AsyncOutputStream {
 public:
  ...
  kj::Promise<void> write(kj::ArrayPtr<const kj::byte> buffer) override {
    return hot(sink->write(buffer.as<kj_rs::Rust>()));
  }
...
class RustWebSocket final: public kj::WebSocket {
 public:
  ...
  kj::Promise<void> send(kj::ArrayPtr<const kj::byte> message) override {
    return hot(session->send(false, message.as<kj_rs::Rust>()));
  }

  kj::Promise<void> send(kj::ArrayPtr<const char> message) override {
    return hot(session->send(true, message.asBytes().as<kj_rs::Rust>()));
  }

  kj::Promise<void> close(uint16_t code, kj::StringPtr reason) override {
    return hot(session->close(code, reason.asBytes().as<kj_rs::Rust>()));
  }
  ...
  kj::Promise<void> whenAborted() override {
    return hot(session->when_aborted());
  }
...
class RustTunnelStream final: public kj::AsyncIoStream {
 public:
  ...
  kj::Promise<void> write(kj::ArrayPtr<const kj::byte> buffer) override {
    return hot(tunnel->write(buffer.as<kj_rs::Rust>()));
  }
  ...
  kj::Promise<void> whenWriteDisconnected() override {
    return hot(tunnel->when_write_disconnected());
  }
...
class RustWebSocketPipeEnd final: public kj::WebSocket {
 public:
  ...
  kj::Promise<void> send(kj::ArrayPtr<const kj::byte> message) override {
    return hot(end->send_binary(message.as<kj_rs::Rust>()));
  }

  kj::Promise<void> send(kj::ArrayPtr<const char> message) override {
    return hot(end->send_text(message.asBytes().as<kj_rs::Rust>()));
  }

  kj::Promise<void> close(uint16_t code, kj::StringPtr reason) override {
    return hot(end->close(code, reason.asBytes().as<kj_rs::Rust>()));
  }
  ...
  kj::Promise<void> pumpTo(kj::WebSocket& other) override {
    return hot(ws_pipe_pump_to(*end, other));
  }

  kj::Maybe<kj::Promise<void>> tryPumpFrom(kj::WebSocket& other) override {
    return hot(ws_pipe_pump_from(*end, other));
  }
