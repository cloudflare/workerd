...
async fn write_then_drop(mut io: ServeIo, done_tx: watch::Sender<bool>) -> std::io::Result<u64> {
    ...
    // With a peer that never reads, this write blocks once the duplex (and the kj stream's own
    // buffering) is full -- which is the situation under test -- so give up on it after a
    // moment and drop the stream mid-write.
