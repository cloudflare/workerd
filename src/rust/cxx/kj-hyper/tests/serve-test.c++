...
KJ_TEST("take_kj_socket on an already-unwrapped (hollow) kj-rs-io stream is refused") {
  ...
  kj_rs_io::unwrapTokioStream(*pair.server);
...
KJ_TEST("serve_kj_stream pump: a consumer that drops while its outbound write is blocked "
        "flushes like close(2) and ends when the peer disconnects") {
  // The kj peer never reads, so the pump's consumer->kj write blocks inside the kj stream and
  // the consumer drops its end mid-write without shutdown(). close(2) semantics: the bytes the
  // consumer already wrote keep flushing to the peer -- nothing is discarded -- so the pump
  // stays pending while the peer neither reads nor goes away, and settles as soon as the peer
  // disconnects. It never waits on the vanished consumer for anything.
  auto io = setupTokioAsyncIo();
  auto &ws = io.getWaitScope();
  auto pipe = kj::newTwoWayPipe();
  ...
  io.getTimer().afterDelay(50 * kj::MILLISECONDS).wait(ws);
  KJ_EXPECT(!drive.poll(ws), "still flushing to a peer that is not reading");
  // The peer goes away: the pending write fails DISCONNECTED and the pump is done.
  pipe.ends[1] = nullptr;
  boundedBy(io, kj::mv(drive), 10 * kj::SECONDS, "the pump to settle once the peer disconnects")
      .wait(ws);
}
