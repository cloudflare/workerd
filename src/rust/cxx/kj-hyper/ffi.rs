...
mod bridge {
    ...
    // =====================================================================================
    // Serving a kj stream natively (serve.rs): taking a kj-rs-io stream's socket, and the
    // bridged operations on a foreign kj::AsyncIoStream (kj-stream.h).

    #[namespace = "kj_rs_io"]
    extern "Rust" {
        /// kj-rs-io's native stream, as `release_tokio_stream` hands it over.
        type TokioStream = kj_rs_io::TokioStream;
    }

    unsafe extern "C++" {
        include!("kj-hyper/kj-stream.h");

        /// Whether the stream is a kj-rs-io wrapper, the only kind `release_tokio_stream` accepts.
        #[cxx_name = "isTokioStream"]
        fn is_tokio_stream(stream: &AsyncIoStream) -> bool;

        /// Takes the native stream out of a kj-rs-io wrapper and destroys the wrapper. Only
        /// valid after `is_tokio_stream`; `TokioStream::into_socket` then decides whether the
        /// socket can be taken, handing the stream back for `wrap_tokio_stream` if not.
        ...
        // and the shared ownership means the stream lives exactly as long as either end does.
        // All returned futures must be polled on the KJ event-loop thread owning the stream.
    ...
    extern "Rust" {
        ...
        /// # Safety
        /// `table` must outlive the returned client (the same contract as
        /// `new_hyper_http_client`). Fails only if `stream` is a kj-rs-io wrapper whose socket
        /// cannot be handed over right now (an I/O operation still borrows it).
    ...
    extern "Rust" {
        ...
        /// Take the socket out of the accepted stream (`crate::serve::take_kj_socket`: kj-rs-io
        /// streams give up their native tokio socket and their wrapper is consumed; other
...
// object (kj-stream.h) that shares ownership of the stream, and drives it through
// `Pin<&mut _>`, so an in-flight operation's future exclusively borrows its half (a second
// overlapping read is a compile error). The stream itself is destroyed by C++ when the last end
...
impl KjStreamReadHalf {
    ...
    pub(crate) async fn try_read(
        ...
        buf: &mut [u8],
        min_bytes: usize,
    ) -> Result<usize, KjException> {
        // SAFETY: `end` and `buf` are exclusively borrowed for the whole call (the bridge
        // requires the explicit lifetime); the C++ end object outlives the promise because the
        // half owns it.
        unsafe { kj_read_end_try_read(self.end.as_mut(), buf, min_bytes) }.await
    }
}

impl KjStreamWriteHalf {
    /// `kj::AsyncIoStream::write(buffer)` (write-all semantics).
    pub(crate) async fn write(&mut self, buf: &[u8]) -> Result<(), KjException> {
        // SAFETY: as in `KjStreamReadHalf::try_read`.
        unsafe { kj_write_end_write(self.end.as_mut(), buf) }.await
    }
