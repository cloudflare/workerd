...
mod bridge {
    ...
    // =====================================================================================
    // Serving a kj stream natively (serve.rs): kj-rs-io's unwrap hook and the bridged
    // operations on a foreign kj::AsyncIoStream (kj-rs-io/bridge.h).

    #[namespace = "kj_rs_io"]
    extern "Rust" {
        /// kj-rs-io's native stream, as `unwrapTokioStream` hands it back.
    ...
    unsafe extern "C++" {
        ...
        /// Downcasts to the kj-rs-io wrapper and moves the native stream out, leaving the
        /// wrapper hollow. Throws (surfaced as `Err`) for foreign streams, an already-hollow
        /// wrapper, or a stream with I/O in flight.
        #[cxx_name = "unwrapTokioStream"]
        fn unwrap_tokio_stream(stream: Pin<&mut AsyncIoStream>) -> Result<Box<TokioStream>>;
...
// object (kj-rs-io/bridge.h) that shares ownership of the stream, and drives it through
