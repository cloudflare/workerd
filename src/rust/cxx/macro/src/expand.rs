...
use syntax::Pair;
use syntax::Signature;
use syntax::Struct;
use syntax::Trait;
use syntax::Type;
use syntax::TypeAlias;
use syntax::Types;
use syntax::atom::Atom::*;
use syntax::attrs::OtherAttrs;
use syntax::attrs::{self};
use syntax::cfg::CfgExpr;
use syntax::check;
use syntax::file::Module;
use syntax::instantiate::ImplKey;
...
use crate::tokens::ReceiverType;
use crate::tokens::ReceiverTypeSelf;
...
pub fn bridge(mut ffi: Module) -> Result<TokenStream> {
    ...
    let trusted = syntax::Trusted::from_option(&ffi.unsafety);
...
fn expand_cxx_function_decl(efn: &ExternFn, types: &Types) -> TokenStream {
    ...
    let args = efn.args.iter().map(|arg| {
        ...
        let ty = expand_extern_type(&arg.ty, types, ExternTypeStyle::Proper);
    ...
    let outparam = if indirect_return(efn)
        && let Some(ret) = &efn.ret
    {
        let ret = expand_extern_type(ret, types, ExternTypeStyle::Proper);
        Some(quote!(__return: *mut #ret))
    } else {
        None
    };
...
fn expand_cxx_function_shim(efn: &ExternFn, types: &Types) -> TokenStream {
    ...
    let call = if indirect_return && let Some(ret) = &efn.ret {
        let ret = expand_extern_type(ret, types, ExternTypeStyle::Proper);
...
fn expand_rust_function_shim_impl(
    ...
    let args = sig.args.iter().map(|arg| {
        let var = &arg.name.rust;
        let colon = arg.colon_token;
        let ty = expand_extern_type(&arg.ty, types, ExternTypeStyle::Layout);
...
fn expand_rust_function_shim_super(
    ...
    let allow_unused_unsafe = if unsafety.is_some() {
        ...
        Some(quote_spanned!(span=> #[allow(unused_unsafe)]))
    } else {
        None
    };
...
fn expand_kj_rc(_key: NamedImplKey, _types: &Types, _explicit_impl: Option<&Impl>) -> TokenStream {
    TokenStream::new()
}
...
#[derive(Copy, Clone)]
enum ExternTypeStyle {
    Proper,
    Layout,
}

fn expand_extern_type(ty: &Type, types: &Types, style: ExternTypeStyle) -> TokenStream {
    match ty {
        Type::Ident(ident) if ident.rust == RustString => {
        ...
        Type::RustBox(ty) | Type::UniquePtr(ty) => {
            ...
            if matches!(style, ExternTypeStyle::Proper)
                && types.is_considered_improper_ctype(&ty.inner)
            {
                quote_spanned!(span=> *mut ::cxx::core::ffi::c_void)
            } else {
                let inner = expand_extern_type(&ty.inner, types, style);
        ...
        Type::RustVec(ty) => {
            ...
            let elem = expand_extern_type(&ty.inner, types, style);
        ...
        Type::Ref(ty) => {
            ...
            match &ty.inner {
                ...
                inner
                    if matches!(style, ExternTypeStyle::Proper)
                        && types.is_considered_improper_ctype(inner) =>
                {
