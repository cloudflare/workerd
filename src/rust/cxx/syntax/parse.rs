...
use syn::Result;
use syn::ReturnType;
use syn::Signature as RustSignature;
use syn::Token;
use syn::TraitBound;
...
use syn::Variant as RustVariant;
use syn::Visibility;
...
fn parse_struct(cx: &mut Errors, mut item: ItemStruct, namespace: &Namespace) -> Result<Api> {
    ...
    for field in named_fields.named {
        let Some(ident) = field.ident else {
            cx.error(field, "named struct field is missing a name");
            continue;
        };
        let mut cfg = CfgExpr::Unconditional;
        let mut doc = Doc::new();
        ...
        let Some(colon_token) = field.colon_token else {
            cx.error(ident, "named struct field is missing a colon");
            continue;
        };
        fields.push(Var {
            cfg,
...
fn parse_foreign_mod(
    ...
    let trusted = trusted.or_option(foreign_mod.unsafety.as_ref());
    ...
    if let (Some(single_type), None) = (types.next(), types.next()) {
        ...
        for item in &mut items {
            if let Api::CxxFunction(efn) | Api::RustFunction(efn) = item
                && let Some(receiver) = &mut efn.receiver
...
fn parse_extern_verbatim(
    ...
    (|input: ParseStream| -> Result<Api> {
        ...
        } else if input.peek(Token![unsafe]) || input.peek(Token![fn]) {
...
fn pair(
    ...
    Pair {
        namespace,
        cxx: cxx.unwrap_or_else(|| ForeignName::from_ident(default)),
