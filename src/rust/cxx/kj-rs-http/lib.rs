...
// streams.rs (`AsyncInputStream`/`AsyncOutputStream`) — re-exported through the islands so the
...
// paths are unchanged. Kept private here to avoid introducing new top-level `kj::{headers, service,
// streams}` paths.
mod headers;
mod service;
mod streams;
