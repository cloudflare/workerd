...
/// format).
///
/// Construction emits the start line (unless empty — kj's `serialize(nullptr, ...)`
...
/// Create a serializer.
///
/// If `word1` is non-empty, emits the start line `word1 word2 word3\r\n`
...
///
/// It must not contain whitespace (which would introduce
...
/// status line.
///
/// It must not contain CR, LF, or NUL, which would allow injecting additional
...
/// region in place.
///
/// On `ok`, each span is one header's name/value byte ranges, NUL-terminated
/// in-buffer.
...
/// `trimHeaderEnding`, and `end` points at its '\0' sentinel.
///
/// The caller passes the region
...
fn consume_int_range(value: &[u8], pos: &mut usize, content_length: u64) -> Option<HttpRangeSpan> {
    ...
    if let Some(mut last_pos) = maybe_last_pos {
...
/// kj's `tryParseHttpRangeHeader`.
///
/// Returns the satisfiable ranges (`kind` 0), kj's `HttpEverythingRange` (`kind` 1) if a range
/// spec covered the full body, or kj's `HttpUnsatisfiableRange` (`kind` 2) if the ranges
/// specifier is invalid or nothing is satisfiable.
