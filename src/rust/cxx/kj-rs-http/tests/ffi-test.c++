...
KJ_TEST("http connect settings") {
  ...
  // A bridged promise is cold until polled: the mock's connect() -- which installs the TLS
  // starter -- runs on the first poll, so settle the round trip before inspecting its effect.
