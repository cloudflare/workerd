...
impl StackString {
    /// Creates uninitialized storage for a C++ string.
    pub fn new() -> Self {
        Self {
    ...
    /// Initializes the storage and returns its pinned C++ string.
    ///
    /// # Safety
    /// The returned reference must not outlive this `StackString`.
