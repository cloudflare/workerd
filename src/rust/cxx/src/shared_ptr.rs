...
use crate::weak_ptr::WeakPtrTarget;
    ...
    T: SharedPtrTarget,
{
    ...
    pub fn null() -> Self {
        let mut shared_ptr = MaybeUninit::<Self>::uninit();
...
// Safety: the target's Send and Sync bounds make shared ownership safe across threads.
unsafe impl<T> Send for SharedPtr<T> where T: Send + Sync + SharedPtrTarget {}
// Safety: the target's Send and Sync bounds make shared access safe across threads.
