...
use crate::cxx_vector::CxxVector;
use crate::cxx_vector::VectorElement;
...
// Safety: exclusive ownership may cross threads when the target is Send.
unsafe impl<T> Send for UniquePtr<T> where T: Send + UniquePtrTarget {}
// Safety: shared access is thread-safe when the target is Sync.
...
// Safety: CxxVector's bridge shims implement the UniquePtrTarget ownership contract.
