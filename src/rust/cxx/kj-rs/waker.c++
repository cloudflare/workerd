...
kj::Promise<void> CrossThreadWakeSink::drainLoop(kj::Arc<CrossThreadWakeSink> sink) {
  ...
  for (;;) {
    ...
    for (auto& cell: sink->takePending()) {
      // Same-thread now: release the cell's queue claim, then arm the event (or no-op if it was
      // destroyed meanwhile). Clearing before arming lets a concurrent foreign wake claim the
      // next replay slot without losing it -- see FutureWakerCell::replayFromSink().
      cell->replayFromSink();
    }
  }
}
...
void FutureWakerCell::wakeByRef() const {
  if (executor->isCurrent()) {
    ...
    // the reference. Coalesce per cell: the first wake to claim the `queued` flag enqueues one
    // strong reference; redundant wakes arriving before the drain replays it are dropped (the
    // Waker contract permits collapsing them, and the eventual single replay + re-poll observes
    // whatever state they signalled). Bounds retained refs and replay work to one per cell rather
    // than one per wake.
