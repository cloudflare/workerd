...
KJ_TEST("RustFuture::lazily() opts out: nothing runs until the promise is first awaited") {
  ...
  // A cold adapter does not capture an event loop until it is first polled: the loop is only
  // constructed HERE, after lazily(), yet construction above did not throw "No event loop".
  kj::EventLoop loop;
  kj::WaitScope waitScope(loop);
...
KJ_TEST("Repeated foreign wakes queue one replay per waker cell") {
  ...
  // One future, four clones of its (single) waker cell, parked Pending.
  auto promise = new_stash_wakers_future_void(4);
  KJ_EXPECT(!promise.poll(waitScope));

  // Hammer the clones with thousands of wakes WITHOUT turning the loop, so the drain never runs
  // and the pending queue is whatever the enqueue path left behind.
  start_wake_storm(4, 2000);
  join_wake_storm();

  // All those wakes target the same cell, so coalescing must collapse them to a single pending
  // slot -- not one Arc per wake. (Before coalescing this observed tens of thousands.)
  auto sink = ::kj_rs::CrossThreadWakeSink::forCurrentLoop();
  KJ_EXPECT(sink->getPendingCountForTest() == 1, sink->getPendingCountForTest(),
      "duplicate cross-thread wakes must coalesce to one replay per cell");
