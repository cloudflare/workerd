...
FuturePollEvent::FuturePollEvent(kj::SourceLocation location)
    : Event(location),
      ...
      // next to the coroutine frame and promise nodes already being allocated.
      wakerCell{kj::arc<FutureWakerCell>(static_cast<kj::_::Event&>(*this))} {
  ensureCrossThreadWakeArmed();
}
...
FuturePollEvent::~FuturePollEvent() noexcept(false) {
  ...
  // on a later same-thread wake and is a safe no-op, rather than arming this freed event. The
  // cross-thread path neutralizes independently: `crossThreadWakePromise` dies with us, making
  // fulfills of the cell's retained fulfiller no-ops.
...
void FuturePollEvent::ensureCrossThreadWakeArmed() {
  if (!wakerCell.cell->needsFreshCrossThreadFulfiller()) {
    return;
  }
  auto paf = kj::newPromiseAndCrossThreadFulfiller<void>();
  crossThreadWakePromise = paf.promise
                               .then([this]() {
    // A foreign thread woke the cell: arm the event from the owning thread, exactly as a
    // same-thread wake would. `this` is safe: the promise chain running this continuation is
    // owned by the event itself. No renewal here (see ensureCrossThreadWakeArmed's declaration);
    // further cross-thread wakes before the next poll coalesce into the pending poll.
    armDepthFirst();
    if (futurePollArmNudge != nullptr) {
      futurePollArmNudge();
    }
  }).eagerlyEvaluate(nullptr);
  wakerCell.cell->replaceCrossThreadFulfiller(kj::mv(paf.fulfiller));
}
