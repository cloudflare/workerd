...
kj::Arc<FutureWakerCell> FuturePollEvent::cloneWakerCell() {
  if (wakerCell.cell.get() == nullptr) {
    // Created lazily on first clone rather than eagerly in the constructor: PollWaker calls this
    // from its own constructor, on the owning loop, BEFORE handing even a borrowed `&Waker` to
    // Rust -- so the cell still exists in time for the first poll to clone or wake it from any
    // thread (`&Waker` is Sync). Deferring to here is what keeps constructing a cold RustFuture
    // adapter (`lazily()`) independent of any event loop until it is actually polled.
