...
class CrossThreadWakeSink final: public kj::AtomicRefcounted {
 public:
  ...
  // Test-only: the mutex-protected pending-queue length, so coalescing tests can assert
  // deterministically that redundant foreign wakes collapse to one slot per cell.
...
class FutureWakerCell final: public kj::AtomicRefcounted {
 public:
  ...
  // wakeByRef() reads it -- or after the owning loop has been torn down (`!isLive()`, so no thread
  // can take the same-thread read path any more). Testing the OWNER's lifetime rather than the
  // current thread's loop state is what makes that precise: a live owner elsewhere still fails the
  // check even from a loopless thread. KJ promises are single-loop, which makes this true by
 ...
 private:
  ...
  // Owning thread only: release this cell's queue claim, then replay the wake. Clearing the claim
  // BEFORE arming is what makes coalescing lossless: a foreign wake racing this replay re-claims
  ...
  // True while the sink already owns a pending replay for this cell. Set by the first foreign wake
  // to reach the sink and cleared just before the drain replays it (replayFromSink), so redundant
  // wakes in between claim no additional strong reference or replay work. The Rust Waker contract
  // permits this coalescing.
