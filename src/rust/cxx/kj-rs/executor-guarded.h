...
class ExecutorGuarded {
 public:
  ...
  ~ExecutorGuarded() noexcept(false) {
    // Teardown-tolerant: if our owning event loop is already gone (`!isLive()`), proceed quietly
    // (best-effort destruction of `value`). Destruction can legitimately run after loop teardown
    // from Rust drop glue (e.g. the tokio runtime cancelling still-pending LocalSet tasks in
    // TokioPort::drop, after TokioAsyncIoContext destroyed the WaitScope and EventLoop); a throw
    // there would unwind through a cxx `prevent_unwind` boundary and abort the process. We test the
    // OWNER's lifetime (`isLive()`), not the current thread's loop state: destroying `value` while
    // the owning loop is still live -- from any other thread, whether or not that thread has a loop
    // of its own -- is the contract violation we want to catch, and it throws.
    KJ_REQUIRE(executor->isCurrent() || !executor->isLive(), "destruction on wrong event loop");
  }
 ...
 private:
  ...
  // could alias — a freed Executor. `addRef()` keeps the Executor at a stable, valid address so
  // both `isCurrent()` and `isLive()` stay safe to call here; if the loop is gone, `isLive()`
  // reports false without throwing and the destructor lets destruction proceed quietly (`get()`
  // still throws, since post-teardown *access* remains a contract violation).
