...
class FuturePollEvent: public kj::_::PromiseNode, public kj::_::Event, public kj::PtrTarget {
 ...
 private:
  ...
  // (PollWaker calls it from its constructor, before exposing any Waker to Rust) -- so a cold
  // RustFuture adapter stays independent of any loop until it is actually polled. The cell then
  // lives until both this event and every Rust reference are gone; ~FuturePollEvent neutralizes it
  // so late wakes no-op.
  ...
  // event's destruction structurally, not by a destructor body remembering to call it. Starts
  // null: the cell is created on the first cloneWakerCell() (first poll), so a never-polled cold
  // adapter carries no cell and neutralizes nothing.
