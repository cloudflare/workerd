...
//!     └── storedWaker — kj::Maybe<rust::Box<RustWaker>>: OUR OWN clone of an arbitrary `Waker`,
//!           owned across the bridge as an ordinary `rust::Box<RustWaker>` (RustWaker just wraps a
//!           `std::task::Waker`) with normal drop glue on both sides; the generic path when polled
//!           by any other runtime — fire() calls its `wake()`
//! ```
#![allow(unsafe_code)]
