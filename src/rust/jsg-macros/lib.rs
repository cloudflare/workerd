...
pub fn jsg_struct(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let empty_fields = syn::punctuated::Punctuated::<syn::Field, syn::token::Comma>::new();
    let named_fields = match &data.fields {
        Fields::Named(fields) => &fields.named,
        Fields::Unit => &empty_fields,
        Fields::Unnamed(_) => {
            return error(&input, "#[jsg_struct] does not support tuple structs");
        }
    };

    let field_assignments = named_fields.iter().filter_map(|field| {
    ...
    let field_extractions = named_fields.iter().filter_map(|field| {
        if !matches!(field.vis, syn::Visibility::Public(_)) {
            return None;
        }
        let field_name = field.ident.as_ref()?;
    ...
    let field_names = named_fields.iter().filter_map(|field| {
        if !matches!(field.vis, syn::Visibility::Public(_)) {
            return None;
        }
        field.ident.as_ref()
...
pub fn jsg_resource(attr: TokenStream, item: TokenStream) -> TokenStream {
        ...
        Data::Struct(data) => match &data.fields {
            ...
            Fields::Unnamed(_) => {
                return syn::Error::new_spanned(
                    &input,
                    "#[jsg::resource] does not support tuple structs",
                )
                .to_compile_error()
                .into();
            }
    ...
    quote! {
        ...
        impl jsg::ResourceTemplate for #template_name {
            fn new(isolate: jsg::v8::IsolatePtr) -> Self {
                Self { constructor: jsg::create_resource_constructor::<#name>(isolate) }
            }
