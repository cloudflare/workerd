...
public:
  ...
  // This function constructs a `std::task::Context` in Rust wrapping the given `RootWaker`. It
  // then calls the future's `Future::poll()` trait function.
  //
  // The reason we pass a `const RootWaker&`, and not the more generic `const CxxWaker&`, is
  // because `RootWaker` exposes an API which Rust can use to optimize awaiting KJ Promises inside
  // of this future.
  //
  ...
  bool poll(const RootWaker& waker) noexcept;
