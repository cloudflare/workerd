...
public:
  ...
  // This function constructs a `std::task::Context` in Rust wrapping the given `AwaitWaker`. It
  // then calls the future's `Future::poll()` trait function.
  //
  // The reason we pass a `const AwaitWaker&`, and not the more generic `const CxxWaker&`, is
  // because `AwaitWaker` exposes an API which Rust can use to optimize awaiting KJ Promises inside
  // of this future.
  //
  ...
  bool poll(const AwaitWaker& waker) noexcept;
