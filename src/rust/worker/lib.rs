...
mod tests {
    ...
    #[async_trait::async_trait(?Send)]
    impl kj::http::HttpService for HeaderOverrideProxy {
        async fn request<'a>(
            &'a mut self,
            method: kj::http::HttpMethod,
            url: &'a [u8],
            headers: kj::http::HttpHeadersRef<'a>,
            request_body: std::pin::Pin<&'a mut kj::io::AsyncInputStream>,
            response: kj::http::HttpServiceResponse<'a>,
        ) -> kj::Result<()> {
