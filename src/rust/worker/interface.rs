...
pub trait WorkerInterface {
    ...
    async fn custom_event(&mut self, event: Own<CustomEvent>) -> Result<CustomEventResult>;
}
...
impl WorkerInterface for ExceptionWorkerInterface {
    ...
    async fn custom_event(&mut self, _event: Own<CustomEvent>) -> Result<CustomEventResult> {
        Err(self.error.clone().into())
    }
}
