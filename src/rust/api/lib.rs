...
pub fn register_nodejs_modules(registry: Pin<&mut ffi::ModuleRegistry>) {
    jsg::modules::add_builtin(registry, "node-internal:dns", |isolate| unsafe {
        let mut isolate = jsg::Lock::from_isolate(isolate);
        let dns_util = DnsUtil { _private: 0 };
        let dns_util = Box::into_raw(Box::new(dns_util));
        let mut dns_util_wrapper = DnsUtilWrapper {
            js: None,
        ...
        // assert_eq!(lhs, rhs);
        jsg::wrap_resource(&mut isolate, dns_util, &mut dns_util_wrapper).into_ffi()
    });
}
