...
- **Eager push:** write() feeds the codec and settles when the codec has
  ...
  not the encoding suite's HWM-0 demand-driven transform). Output waits
  unboundedly in the codec's buffer; a parked read is served as soon as any
  output exists (for deflate: the 2-byte zlib header from the first write,
  the rest at close-time flush).
- **Delivery (#16):** each read takes a bounded piece of the buffered
  output — a default read at most 64 KiB under TypeScript, the
  internal-stream read buffer under C++ (4 KiB; 16 KiB under the
  `updated-auto-allocate-chunk-size` autogate) — and a BYOB read fills its
  view in both. A write's output is never a single JS chunk, however large.
  Output produced before a trailing-junk error goes to a waiting read first
  under TypeScript (one piece; WPT decompression-extra-input); C++ rejects
  the waiting read (#17).
...
| 17 | Output produced before a trailing-junk error, read waiting | read rejects | read gets one piece, then errors | `trailingJunkAfterLargeOutput` |
...
| `draining-reader.js` | TS only (C++ asserts absence): expectedLength undefined; a closed stream's small backlog swept in ONE read with done; a 1 MiB backlog in 64 KiB pieces, the last with done; lock/release |
| `delivery-shape.js` | a 4 MiB single-write output read in bounded pieces (#16), byte-exact; BYOB views filled to their size; two pending reads take consecutive pieces (C++: #14); tee branches get bounded pieces; trailing junk after a large output (#17) |
