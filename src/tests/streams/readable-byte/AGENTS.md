...
| 7 | close() with partially-filled read(view) | close succeeds; read resolves EMPTY view done=FALSE; closed fulfills | TypeError 'Insufficient bytes to fill elements in the given view' from close(), read, and closed (spec) | `closeWithPartiallyFilledView`; on a tee branch it errors that branch alone and its sibling receives every byte (`teeBranchFractionalCloseErrorsBranch`); on a detached body, `closeWithPartiallyFilledViewDetached` |
