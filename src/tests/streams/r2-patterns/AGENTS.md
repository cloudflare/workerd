...
| --- | --- | --- | --- | --- |
| 1 | readAtLeast(min) when close arrives below min | folds the available bytes into a done=false result, then a done read | same (deferred end-of-data settlement; readable-byte ledger #12/#19) | `byobReadAtLeastAutomatic`, `closedByobTeeOnStart` (both tee branches) |
