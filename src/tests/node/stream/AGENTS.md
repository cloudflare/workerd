...
- Source end → stream closes (for a Duplex source, on its readable side's
  ...
  error → `AbortError` whose `cause` is `ERR_STREAM_PREMATURE_CLOSE` — also
  when the source destroys itself from inside the `_read()` the adapter's
  `pull()` reaches through `resume()`. An `'error'` a legacy source emits
  after it has ended is swallowed by the no-op listener the bridge leaves
  behind.
...
- `finished()` and `addAbortSignal()` need the Node.js interop hooks (see
  ...
  `IdentityTransformStream`, `FixedLengthStream`) it errors both halves
  with the `AbortError`, as the pair's controller `error()` would: a parked
  write, later writes, pending reads and both `closed` promises reject with
  it, `finished()` on the other half reports it, and a `pipeThrough()`
  source upstream of the pair is cancelled with it. Neither the sink's
  abort nor the transformer's `cancel` runs. A half whose pair is already
  torn down keeps its first error. In Node the other half never learns and
  its pending operations stay pending.
...
| --- | --- | --- | --- | --- |
| 1 | Locked-stream error text (`getReader`/`getWriter` on a locked stream, reached through fromWeb) | `This ReadableStream is currently locked to a reader.` / `This WritableStream is currently locked to a writer.` | `Cannot get a reader for a stream that is locked` / `Cannot get a writer for a stream that is locked` | `fromWebLocksTheStream`, `fromWebLockedInputThrows`, `writableFromWebLocksTheStream`, `fromWebPairLockedReadableLeavesWriterLocked` |
...
| 5 | Node.js interop hooks (`Symbol.for('nodejs.webstream.isClosedPromise')`, `…controllerErrorFunction`) | absent; `finished()`, `promises.finished()` and `addAbortSignal()` throw `ERR_WEB_STREAM_INTEROP_UNSUPPORTED` up front, as does `compose()` for a composition whose writable side would have to observe a web tail | non-enumerable prototype getter and method; the APIs work as in Node, including on native-backed streams (a `Response` body), with two deliberate differences from Node: `addAbortSignal()` errors byte streams too, where Node's readable hook is a no-op, and on a half of a transform pair it errors both halves, where Node's leaves the other half pending | `finished-and-abort.js`, `abort-transform-pairs.js`, `compose-web.js` |
