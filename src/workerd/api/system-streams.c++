...
kj::Promise<void> EncodedAsyncOutputStream::write(kj::ArrayPtr<const byte> buffer) {
  ...
  // The evalNow() converts a synchronous throw from the tryWriteSync() probe (which the
  // contract permits when a synchronous write is possible but fails) into a rejected promise,
  // preserving write()'s never-throws-synchronously behavior for callers reached directly
  // from JS (e.g. the internal controller's write loop).
  return kj::evalNow([&]() -> kj::Promise<void> {
    if (target.tryWriteSync(buffer)) {
      // Fast path: the write completed synchronously (e.g. into a waiting reader or an
      // in-memory buffer). No pending event registration is needed since we never suspend.
      return kj::READY_NOW;
    }
...
kj::Promise<void> EncodedAsyncOutputStream::write(
  ...
  auto& target = getInner();
  // See the buffer overload above for why the evalNow() is needed.
  return kj::evalNow([&]() -> kj::Promise<void> {
