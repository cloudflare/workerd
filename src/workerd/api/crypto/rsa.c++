...
kj::Array<kj::byte> Rsa::sign(const kj::ArrayPtr<const kj::byte> data) const {
  ...
  OSSLCALL(RSA_decrypt(rsa, &signatureSize,
...
kj::Array<kj::byte> Rsa::cipher(EVP_PKEY_CTX* ctx,
  ...
  KJ_IF_SOME(l, algorithm.label) {
    ...
    JSG_REQUIRE(1 == EVP_PKEY_CTX_set0_rsa_oaep_label(ctx, labelCopy, l.size()),
  ...
  JSG_REQUIRE(1 == encryptDecrypt(ctx, nullptr, &maxResultLength, data.begin(),
        data.size()), DOMOperationError, "Failed to compute length of RSA-OAEP result",
        tryDescribeOpensslErrors());

  kj::Vector<kj::byte> result(maxResultLength);
  auto err = encryptDecrypt(ctx, result.begin(), &maxResultLength, data.begin(),
      data.size());
...
SubtleCrypto::JsonWebKey Rsa::toJwk(KeyType keyType,
...
CryptoKeyPair generateRsaPair(jsg::Lock& js,
  ...
  if (normalizedName == "RSASSA-PKCS1-v1_5") {
    return createPair(kj::heap<RsassaPkcs1V15Key>(kj::mv(publicKeyData),
                                                  kj::mv(keyAlgorithm), true),
                      kj::heap<RsassaPkcs1V15Key>(kj::mv(privateKeyData),
                                                  kj::mv(privateKeyAlgorithm),
                                                  privateKeyExtractable));
  } else if (normalizedName == "RSA-PSS") {
    return createPair(kj::heap<RsaPssKey>(kj::mv(publicKeyData),
                                          kj::mv(keyAlgorithm), true),
                      kj::heap<RsaPssKey>(kj::mv(privateKeyData),
                                          kj::mv(privateKeyAlgorithm),
                                          privateKeyExtractable));
  } else if (normalizedName == "RSA-OAEP") {
    return createPair(kj::heap<RsaOaepKey>(kj::mv(publicKeyData),
                                           kj::mv(keyAlgorithm), true),
                      kj::heap<RsaOaepKey>(kj::mv(privateKeyData),
                                           kj::mv(privateKeyAlgorithm),
                                           privateKeyExtractable));
  }
...
kj::OneOf<jsg::Ref<CryptoKey>, CryptoKeyPair> CryptoKey::Impl::generateRsa(
  ...
  KJ_ASSERT(normalizedName == "RSASSA-PKCS1-v1_5" ||
            normalizedName == "RSA-PSS" ||
