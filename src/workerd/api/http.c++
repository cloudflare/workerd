...
class ActorFetchRetryState {
 public:
  static ActorFetchRetryState create(
      TimerChannel& timer, RequestObserver& observer, SubrequestBodyRewindable bodyRewindable);
...
ActorFetchRetryState ActorFetchRetryState::create(
  ...
  if (bodyRewindable) {
    ...
    // TODO(STOR-5489): Keep sender-side retries disabled until retry-claim enforcement is deployed
    // to every receiver. A mixed fleet can otherwise execute both an ambiguous request and its
    // retry.
...
kj::OneOf<kj::Duration, kj::Exception> ActorFetchRetryState::prepareRetry(
  ...
  if (exception.getDetail(jsg::REQUEST_NOT_DELIVERED_TO_ACTOR_DETAIL_ID) == kj::none) {
    KJ_REQUIRE_NONNULL(metadata).isRetry = IsActorRetry::YES;
  } else if (KJ_REQUIRE_NONNULL(metadata).isRetry == IsActorRetry::NO) {
    metadata = generateActorRetryRequestMetadata(kj::systemCoarseCalendarClock().now());
  }
...
jsg::Promise<jsg::Ref<Response>> fetchImplNoOutputLock(jsg::Lock& js,
    ...
    kj::Vector<kj::Url> urlList) {
  kj::Maybe<ActorFetchRetryState> retryState;
  if (fetcher->supportsActorFetchRetries()) {
    auto& context = IoContext::current();
    retryState = ActorFetchRetryState::create(context.getIoChannelFactory().getTimer(),
        context.getMetrics(), SubrequestBodyRewindable(jsRequest->canRewindBody()));
  }
...
jsg::Promise<jsg::Ref<Response>> fetchImplNoOutputLockAttempt(jsg::Lock& js,
  ...
  auto bodyRewindable = SubrequestBodyRewindable(jsRequest->canRewindBody());
  ioContext.getMetrics().setNextSubrequestBodyRewindable(bodyRewindable);
  ...
  if (headers.isWebSocket()) {
    ...
    if (retryState != kj::none) {
      auto resultPromise = captureActorFetchAttempt(kj::mv(webSocketResponse),
          KJ_ASSERT_NONNULL(retryState).addMetricsRef());
    ...
    if (retryState != kj::none) {
      auto resultPromise = captureActorFetchAttempt(
          kj::mv(responsePromise), KJ_ASSERT_NONNULL(retryState).addMetricsRef());
      return ioContext.awaitIo(js, kj::mv(resultPromise),
          [fetcher = kj::mv(fetcher), jsRequest = kj::mv(jsRequest), urlList = kj::mv(urlList),
