...
class LoopbackServiceStub: public Fetcher {
 public:
  ...
  // TODO(now): When we make Fetcher able to be serialized, we should block serialization here and
  //   throw an error explaining that the loopback stub needs to be specialized with props first.
  //   This isn't strictly needed for security, but developers who are passing fetchers over RPC
  //   really should be aware of props and probably should be setting them.
...
class LoopbackDurableObjectClass: public DurableObjectClass {
 public:
  ...
  // TODO(now): Similarly to LoopbackServiceStub, block serialization wher `DurableObjectClass`
