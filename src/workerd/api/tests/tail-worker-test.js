...
const E = {
  ...
  jsrpcDoSubrequest:
    '{"type":"onset","executionModel":"stateless","spanId":"0000000000000000","scriptTags":[],"info":{"type":"custom"}}{"type":"spanOpen","name":"jsRpcCall","spanId":"0000000000000001"}{"type":"spanOpen","name":"jsRpcCall","spanId":"0000000000000002"}{"type":"spanOpen","name":"durable_object_subrequest","spanId":"0000000000000003"}{"type":"spanOpen","name":"jsRpcCall","spanId":"0000000000000004"}{"type":"spanClose","outcome":"ok"}{"type":"attributes","info":[{"name":"objectId","value":"af6dd8b6678e07bac992dae1bbbb3f385af19ebae7e5ea8c66d6341b246d3328"}]}{"type":"spanClose","outcome":"ok"}{"type":"spanOpen","name":"jsRpcCall","spanId":"0000000000000005"}{"type":"spanClose","outcome":"ok"}{"type":"spanOpen","name":"jsRpcCall","spanId":"0000000000000006"}{"type":"spanClose","outcome":"ok"}{"type":"spanClose","outcome":"ok"}{"type":"spanClose","outcome":"ok"}{"type":"outcome","outcome":"ok","cpuTime":0,"wallTime":0}',
...
const expectedWithPropagation = [
  ...
  // Under USER_SPAN_CONTEXT_PROPAGATION, callees now emit a server-side jsRpcSession span
  // wrapping each invocation. The test's tree builder matches parent-child by numeric
  // span ID scoped by traceId; with both the caller's jsRpcCall:client and each callee's
  // jsRpcSession landing at span ID 1, sibling RPC calls on the same callee worker are
  // attributed to the first invocation. nonFunctionProperty therefore appears nested
  // under getCounter rather than as a sibling. This is a test-infrastructure artefact,
  // not an actual misparented span -- the spans themselves are correctly emitted.
  n(E.jsrpcDoSubrequest, [
    n(E.myActorJsrpc),
