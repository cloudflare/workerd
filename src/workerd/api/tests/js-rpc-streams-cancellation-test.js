...
export class Peer extends WorkerEntrypoint {
  ...
  // Cancels with `reason` exactly as given, so that reasons other than Error objects (or none at
  // all) can be sent across the channel.
  async cancelReadableWith(stream, reason) {
    assert.ok(stream instanceof ReadableStream);
...
// The receiver's reason arrives as the value it was, not as an Error describing it.
export const idleReadableCanceledByReceiverWithStringReason = {
  async test(controller, env) {
    if (!isRunner(env)) return;
    const idle = makeIdleReadable();
    await env.PEER.cancelReadableWith(idle.readable, 'receiver canceled');
    const reason = await idle.whenCanceled;
    assert.strictEqual(idle.cancelCalls, 1);
    assert.strictEqual(reason, 'receiver canceled');
  },
};

export const idleReadableCanceledByReceiverWithObjectReason = {
  async test(controller, env) {
    if (!isRunner(env)) return;
    const idle = makeIdleReadable();
    await env.PEER.cancelReadableWith(idle.readable, {
      code: 42,
      detail: ['a', 'b'],
    });
    const reason = await idle.whenCanceled;
    assert.strictEqual(idle.cancelCalls, 1);
    assert.deepStrictEqual(reason, { code: 42, detail: ['a', 'b'] });
  },
};

// A cancel with no reason arrives as the generic release error rather than as undefined: where
// the cancel reaches the transferred stream's source, a reason that was never given and one that
// could not be serialized look the same.
export const idleReadableCanceledByReceiverWithoutReason = {
  async test(controller, env) {
    if (!isRunner(env)) return;
    const idle = makeIdleReadable();
    await env.PEER.cancelReadableWith(idle.readable, undefined);
    const reason = await idle.whenCanceled;
    assert.strictEqual(idle.cancelCalls, 1);
    assert.ok(reason instanceof Error);
...
export const nativeReadableCanceledByReceiver = {
  async test(controller, env) {
    ...
    // Only the rejection is asserted, not its reason: what the writer sees is the identity
    // stream's own disconnection error, not the receiver's. The identity stream marks itself
    // errored as soon as the pump's pending read on it is dropped, which happens before the pump
    // cancels it with the receiver's reason, and that cancel then finds it already errored.
