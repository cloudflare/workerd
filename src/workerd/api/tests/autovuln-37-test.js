...
// bytesWritten is JS-controlled. On the invalidated-branch path, an out-of-range
// bytesWritten must throw (not trip a C++ assertion): the fix trims the detached
// view by sliced.size() - bytesWritten, a size_t subtraction that would underflow
// for bytesWritten > view size (or negative) and abort the process.
export const teeInvalidatedRespondRejectsOutOfRangeBytesWritten = {
  async test() {
    function makeInvalidatedByobRequest() {
      let ctrl;
      const rs = new ReadableStream({
        type: 'bytes',
        start(c) {
      ...
      const readerA = a.getReader({ mode: 'byob' });
      const readerB = b.getReader({ mode: 'byob' });
      readerA.read(new Uint8Array(8));
      readerB.read(new Uint8Array(8));
      const byobReq = ctrl.byobRequest;
      // Cancel branch A synchronously invalidates its read request while a
      // sibling consumer (branch B) still exists.
      readerA.cancel('done with A');
      return byobReq;
    }

    // bytesWritten larger than the 8-byte view: RangeError, not a crash.
    throws(() => makeInvalidatedByobRequest().respond(9), {
      name: 'RangeError',
    });

    // bytesWritten == 0 while the stream is open: TypeError.
    throws(() => makeInvalidatedByobRequest().respond(0), {
      name: 'TypeError',
    });
  },
};
