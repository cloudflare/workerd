...
export const decodeStreamingTest = {
  test() {
    ...
    ok(decoder.decode(fooCat) === "foo 😺", "foo-cat from Uint8Buffer to be foo 😺");
    ok(decoder.decode(fooCat.buffer) === "foo 😺", "foo-cat from ArrayBuffer to be foo 😺");
    ...
    [twoByteCodePoint, threeByteCodePoint, fourByteCodePoint].forEach((input) => {
    ...
    ok(asciiDecoder.decode().length === 0, "decoded undefined array length to be 0");
    ok(asciiDecoder.decode(new Uint8Array([162, 174, 255])) === '¢®ÿ',
       "decoded extended ascii correctly")

    // Test streaming

    ok(decodeStreaming(fatalDecoder, twoByteCodePoint) === '¢',
       "2-byte code point (cent sign) to be decoded correctly");
    ok(decodeStreaming(fatalDecoder, threeByteCodePoint) === '€',
       "3-byte code point (euro sign) to be decoded correctly");
    ok(decodeStreaming(fatalDecoder, fourByteCodePoint) === '😺',
       "4-byte code point (cat emoji) to be decoded correctly");

    const bom = new Uint8Array([0xef, 0xbb, 0xbf]);
    const bomBom = new Uint8Array([0xef, 0xbb, 0xbf, 0xef, 0xbb, 0xbf]);

    ok(decodeStreaming(fatalDecoder, bom) === '',
       "BOM to be stripped by TextDecoder without ignoreBOM set");
    ok(decodeStreaming(fatalDecoder, bomBom) === '\ufeff',
       "first BOM to be stripped by TextDecoder without ignoreBOM set");

    ok(decodeStreaming(fatalIgnoreBomDecoder, bom) === '\ufeff',
       "BOM not to be stripped by TextDecoder with ignoreBOM set");
    ok(decodeStreaming(fatalIgnoreBomDecoder, bomBom) === '\ufeff\ufeff',
...
export const textEncoderTest = {
  test() {
    ...
    deepStrictEqual(encoder.encode('foo 😺'),
...
export const big5 = {
  test() {
    ...
    const check = new Uint8Array([0xe4, 0xb8, 0xad, 0xe5, 0x9c, 0x8b, 0xe4, 0xba, 0xba]);
    const enc = new TextEncoder();
...
export const utf16leFastTrack = {
  test() {
    ...
    const input = new Uint8Array([0x68, 0x00, 0x65, 0x00, 0x6c, 0x00, 0x6c, 0x00, 0x6f, 0x00]);
