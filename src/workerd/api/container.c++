...
jsg::Promise<void> Container::interceptOutboundHttp(
  ...
  return ioctx.awaitIo(js, interceptOutboundHttpImpl(kj::mv(addr), kj::mv(channel)));
}

kj::Promise<void> Container::interceptOutboundHttpImpl(
...
jsg::Promise<void> Container::interceptAllOutboundHttp(jsg::Lock& js, jsg::Ref<Fetcher> binding) {
  auto& ioctx = IoContext::current();
  auto channel = binding->getSubrequestChannel(ioctx);
  return ioctx.awaitIo(js, interceptAllOutboundHttpImpl(kj::mv(channel)));
}
...
jsg::Promise<void> Container::interceptOutboundHttps(
  ...
  auto& ioctx = IoContext::current();
  auto channel = binding->getSubrequestChannel(ioctx);
  return ioctx.awaitIo(js, interceptOutboundHttpsImpl(kj::mv(addr), kj::mv(channel)));
}

kj::Promise<void> Container::interceptOutboundHttpsImpl(
    kj::String addr, kj::Own<IoChannelFactory::SubrequestChannel> channel) {
  auto token = co_await channel->getToken(IoChannelFactory::ChannelTokenUsage::RPC);
  { auto drop = kj::mv(channel); }  // no longer needed
...
jsg::Promise<void> Container::interceptOutboundTcp(
  ...
  auto& ioctx = IoContext::current();
  auto channel = binding->getSubrequestChannel(ioctx);
  return ioctx.awaitIo(js, interceptOutboundTcpImpl(kj::mv(addr), kj::mv(channel)));
}

kj::Promise<void> Container::interceptOutboundTcpImpl(
    kj::String addr, kj::Own<IoChannelFactory::SubrequestChannel> channel) {
