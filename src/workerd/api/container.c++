...
jsg::Promise<void> Container::setInactivityTimeout(jsg::Lock& js, int64_t duration) {
  JSG_REQUIRE(
      duration < 0, TypeError, "setInactivityTimeout() cannot be called with a duration <= 0");

  auto req = rpcClient->setInactivityTimeoutRequest();

  req.setDurationMs(duration);
