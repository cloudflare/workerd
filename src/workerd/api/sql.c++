...
jsg::Ref<SqlStorage::Cursor> SqlStorage::exec(
  ...
  auto userSpan = IoContext::current().makeUserTraceSpan("durable_object_storage.exec"_kjc);
...
SqlStorage::IngestResult SqlStorage::ingest(jsg::Lock& js, kj::String querySql) {
  auto userSpan = IoContext::current().makeUserTraceSpan("durable_object_storage.ingest"_kjc);
  SqliteDatabase::Regulator& regulator = *this;
...
double SqlStorage::getDatabaseSize(jsg::Lock& js) {
  auto userSpan =
      IoContext::current().makeUserTraceSpan("durable_object_storage.getDatabaseSize"_kjc);
  auto& db = getDb(js);
