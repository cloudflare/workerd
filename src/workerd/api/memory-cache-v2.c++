...
static FallbackDoneCallback makeFallback(
  ...
  return [permit = kj::heap<FallbackPermitOwner>(kj::mv(permit)), key = kj::mv(key)](
    ...
    KJ_IF_SOME(value, result) {
      ...
      auto expiration = value.expiration.orDefault(0);
      KJ_IF_SOME(exception, kj::runCatchingExceptions([&]() {
        auto trace = currentPermit->succeed(
            asRustBytes(source), value.expiration != kj::none, expiration, cacheNow());
  ...
  class V2 final: public MemoryCacheNamespace {
   public:
    explicit V2(MemoryCachePolicy policy)
        : cacheNamespace(rustCache::namespace_new(
              policy.maxTotalValueSize != kj::none, policy.maxTotalValueSize.orDefault(0))) {}
