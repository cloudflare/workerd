...
class WorkflowEntrypoint: public jsg::Object {
 public:
  ...
  // Called by the Workflows engine instead of run(). Monkey-patches step.do to
  // wrap user callbacks with tracing spans, then delegates
  // to this.run(event, step).
  jsg::Promise<jsg::Value> runStep(jsg::Lock& js, jsg::Value event, jsg::Value step);
