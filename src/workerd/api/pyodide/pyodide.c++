...
kj::String getPyodidePackagePath(kj::StringPtr packagesVersion, kj::StringPtr filename) {
  return kj::str("python-package-bucket/", packagesVersion, "/", filename);
}
...
kj::Array<kj::String> getPythonPackageFiles(kj::StringPtr lockFileContents,
  ...
  for (const auto& ent: *packages) {
    ...
    KJ_IF_SOME(row, maybeRow) {
      ...
      // TODO: See above, TL;DR remove this once we release new Python/package version.
      if (packagesVersion == "20240829.4") {
        auto packageType = getField(obj, "package_type").getString();
        if (maybeRow == kj::none && packageType == "cpython_module") {
          res.add(kj::mv(fileName));
        }
      }
    }
  ...
  JSG_REQUIRE(allRequirements.size() == 0, Error,
...
kj::Promise<void> loadPyodidePackage(const PythonConfig& pyConfig,
  ...
  auto path = getPyodidePackagePath(packagesVersion, filename);
  ...
  KJ_IF_SOME(body, maybeBody) {
    ...
    // Store the decompressed data in memory
    pyodidePackageManager.setPyodidePackageData(kj::str(path), kj::mv(decompressed));
  } else {
...
void fetchPyodidePackages(const PythonConfig& pyConfig,
  ...
  // Get the lock file for this package version
  ...
  // Get the filenames of packages to fetch
  auto filenames =
      getPythonPackageFiles(KJ_ASSERT_NONNULL(pyodideLock), pythonRequirements, packagesVersion);

  // Start a thread for package fetching
  kj::Thread([&pyConfig, &pyodidePackageManager, filenames = kj::mv(filenames),
                 packagesVersion = kj::str(packagesVersion)]() mutable {
    ...
    // Create a list of promises for each package download
    kj::Vector<kj::Promise<void>> promises;

    for (const auto& filename: filenames) {
      // Add each package loading promise to our list
      promises.add(loadPyodidePackage(
          pyConfig, pyodidePackageManager, packagesVersion, filename, *client, timer, table));
    }

    // Wait for all package downloads to complete
    if (!promises.empty()) {
