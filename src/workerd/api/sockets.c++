...
jsg::Promise<void> Socket::close(jsg::Lock& js) {
  ...
  return openedPromiseCopy.whenResolved(js)
      ...
      .then(js,
          JSG_VISITABLE_LAMBDA((self = JSG_THIS), (self),
              (jsg::Lock & js) mutable {
                // Forcibly abort the readable/writable streams. Operations still queued on them
                // (a pipe's pending close, for instance) are rejected with this reason, so it must
                // be a real error rather than the undefined that a reasonless abort() produces.
