...
class WritableStreamSink: public kj::PtrTarget {
 public:
  ...
  // returned promise must not outlive the sink.
...
// rejectWhenWriteDisconnected<T>(sink))`. A rejection of the underlying promise is also treated
// as a disconnection, since either way the sink is not going to accept further writes.
template <typename T>
...
// Captures the current context's identity for stream objects without retaining a reference that
// could be destroyed from a different thread during garbage collection.
