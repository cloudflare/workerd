...
class ReadableStreamJsController final: public ReadableStreamController {
 ...
 private:
  ...
  kj::Maybe<IoContext::Id> ioContext = IoContext::tryGetCurrentId();
...
ReadableStreamJsController::ReadableStreamJsController(): ioContext(tryGetIoContextId()) {}
...
kj::Promise<void> pumpToImpl(IoContext& ioContext,
  ...
  // settled or its cancel is under way. The cancel is best-effort: it rejects if the stream
  ...
  KJ_CATCH(exception) {
    ...
    co_await ioContext.run([&reader, &pumpSettled, ex = exception.clone()](jsg::Lock& js) mutable {
      auto& ioContext = IoContext::current();
      // A drop while the cancel below is still pending must not make the teardown defer start a
      // second one, so claim the suppression now rather than after the await.
      pumpSettled = true;
...
jsg::Promise<T> ReadableStreamJsController::readAll(jsg::Lock& js, uint64_t limit) {
  ...
  const auto readAll = [this, limit, stripBom](auto& js) -> jsg::Promise<T> {
    ...
    return maybeAddFunctor(js, kj::mv(promise),
        ...
        // the promise settles.
        [reader = kj::mv(reader)](jsg::Lock& js, T result) -> jsg::Promise<T> {
...
// ======================================================================================
WritableStreamJsController::WritableStreamJsController(): ioContext(tryGetIoContextId()) {}
...
jsg::Promise<void> WritableStreamJsController::pipeLoop(jsg::Lock& js) {
  ...
  auto onFailure = [this, ref = addRef()](jsg::Lock& js, jsg::Value value) mutable {
  ...
  KJ_IF_SOME(writableController, tryGetWritableController()) {
    ...
    if (backpressure) {
      ...
      return KJ_ASSERT_NONNULL(maybeBackpressureChange)
          ...
          .then(js,
              [chunkRef = kj::mv(chunkRef), ref = JSG_THIS](
