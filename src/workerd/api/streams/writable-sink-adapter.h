...
class WritableStreamSinkJsAdapter final {
 ...
 private:
  ...
  // holds the both the underlying WritableStreamSink and the write queue.
  // It must be held within an IoOwn.
  struct Active;
  struct Closed final {};
  kj::OneOf<IoOwn<Active>, Closed, kj::Exception> state;

  // Used for back pressure signaling. When backpressure is indicated, the
  ...
  class BackpressureState final {
   ...
   private:
    ...
    friend class WritableStreamSinkJsAdapter;
