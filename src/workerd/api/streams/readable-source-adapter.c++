...
jsg::Promise<jsg::JsRef<jsg::JsArrayBuffer>> ReadableStreamSourceJsAdapter::readAllBytes(
  ...
  return ioContext
    ...
    KJ_IF_SOME(result, holder->result) {
      ...
      // v8 sandboxing rules.
      return jsg::JsArrayBuffer::create(js, result).addRef(js);
    } else {
      return jsg::JsArrayBuffer::create(js, 0).addRef(js);
    }
  })
      .catch_(js,
          [self = selfRef.addRef()](
...
void ReadableSourceKjAdapter::Active::cancel(kj::Exception reason) {
  ...
  if (!wasDone) {
    ...
    // cancel it here. The adapter may outlive its IoContext, in which
    // case there is no isolate in which to run the cancellation.
