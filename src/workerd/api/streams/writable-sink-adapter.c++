...
void WritableStreamSinkKjAdapter::Active::abort(kj::Exception reason) {
  ...
  // The adapter may outlive its IoContext, in which case there is no isolate
  // in which to run the abort algorithm.
  KJ_IF_SOME(ctx, ioContext) {
    ctx->addTask(
        ctx->run([writable = kj::mv(stream), writer = kj::mv(writer), exception = reason.clone()](
