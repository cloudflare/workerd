...
KJ_TEST("Basic construction with default options") {
  ...
  fixture.runInIoContext([&](const TestFixture::Environment& env) {
    auto sink = newIoContextWrappedWritableSink(
        env.context.getWeakRef(), newWritableSink(newNullOutputStream()));
...
KJ_TEST("WritableStreamSinkKjAdapter construction") {
  ...
  fixture.runInIoContext([&](const TestFixture::Environment& env) {
    ...
    auto adapter =
        kj::heap<WritableStreamSinkKjAdapter>(env.js, env.context.getWeakRef(), kj::mv(stream));
  });
}

KJ_TEST("WritableStreamSinkKjAdapter may outlive its IoContext") {
  capnp::MallocMessageBuilder message;
  auto flags = message.initRoot<CompatibilityFlags>();
  flags.setStreamsJavaScriptControllers(true);
  TestFixture fixture({.featureFlags = flags.asReader()});
  WritableStreamContext context;
  kj::Maybe<kj::Own<WritableStreamSinkKjAdapter>> adapter;
  ...
  // Destroying the adapter must not try to abort through the dead IoContext.
  fixture.runInIoContext([&](const TestFixture::Environment&) { adapter = kj::none; });
}
