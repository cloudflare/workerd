...
KJ_TEST("WritableStreamSinkKjAdapter concurrent writes forbidden") {
  ...
  fixture.runInIoContext([&](const TestFixture::Environment& env) {
    ...
    try {
      auto p2 KJ_UNUSED = adapter->write(buffer.asPtr());
      KJ_FAIL_ASSERT("Second concurrent write should have thrown");
    } catch (...) {
      auto ex = kj::getCaughtExceptionAsKj();
      KJ_ASSERT(ex.getDescription().contains("Cannot have multiple concurrent writes"));
      return p1.then([&adapter = *adapter]() { return adapter.end(); }).attach(kj::mv(adapter));
    }
  });
}
...
KJ_TEST("WritableStreamSinkKjAdapter concurrent writes forbidden") {
  ...
  fixture.runInIoContext([&](const TestFixture::Environment& env) {
    ...
    return adapter->end()
      ...
      try {
        auto p2 KJ_UNUSED = adapter.write(buffer.asPtr());
        KJ_FAIL_ASSERT("Write after end should have thrown");
      } catch (...) {
        auto ex = kj::getCaughtExceptionAsKj();
