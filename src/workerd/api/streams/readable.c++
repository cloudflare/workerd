...
class RpcStreamCancelerState final: public kj::Refcounted {
 public:
  explicit RpcStreamCancelerState(rpc::StreamCanceler::Client canceler)
 ...
 private:
  kj::Maybe<rpc::StreamCanceler::Client> canceler;
  uint liveSources = 0;
  // The most recent reason a source was canceled with -- the JavaScript value the cancel carried,
  // in the serialized form it travels in within the exception -- held until the last source is
  // done and it can be sent.
  kj::Maybe<kj::Array<kj::byte>> serializedReason;

  // A reason that began as a JavaScript value carries that value, serialized, as the exception's
  // jsg::TUNNELED_EXCEPTION_DETAIL_ID detail (attached when the value was converted to a
  // kj::Exception), so forwarding it needs neither the isolate lock nor any JavaScript, and this
  // runs from KJ-side teardown as readily as from a JavaScript-initiated cancel(). An exception
  // without that detail -- a cancel() with no reason, a reason that could not be serialized, or a
  // failure that originated in KJ -- sends no reason, and the origin synthesizes one. The reason
  // is subject to the same size limit as any other value sent over JS RPC; one that exceeds it is
  // left out likewise.
  void rememberReason(const kj::Exception& reason) {
    if (canceler == kj::none) return;
    KJ_IF_SOME(detail, reason.getDetail(jsg::TUNNELED_EXCEPTION_DETAIL_ID)) {
      if (detail.size() <= MAX_JS_RPC_MESSAGE_SIZE) {
        serializedReason = kj::heapArray<kj::byte>(detail);
      }
    }
  }
...
class RpcReadableStreamSource final: public ReadableStreamSource {
 public:
  ...
  // EOF is observed as a short read (in pumpTo(), below, as the pump completing). A consumer that
  // reads exactly the stream's length and stops never observes it, so it keeps the canceler -- and
  // with it the RPC session -- until this object is destroyed; the cancel sent then is ignored by
  // the origin, whose pump has already completed and released the signal it would have fired.
  kj::Promise<size_t> tryRead(void* buffer, size_t minBytes, size_t maxBytes) override {
    return inner->tryRead(buffer, minBytes, maxBytes)
...
kj::Own<ReadableStreamSource> newRpcReadableStreamSource(
  ...
  if (reader.hasCanceler() &&
    ...
    source = kj::heap<RpcReadableStreamSource>(
        kj::mv(source), kj::rc<RpcStreamCancelerState>(reader.getCanceler()));
  }
  return kj::mv(source);
}
