...
kj::Own<kj::HttpClient> KvNamespace::getHttpClient(IoContext& context,
  ...
  auto client = context.getHttpClientWithCallback(
      subrequestChannel, true, kj::none, operationName, [&](TraceContext& tracing) {
    auto& userSpan = tracing.userSpan;
    userSpan.setTag("db.system"_kjc, kj::str("cloudflare-kv"_kj));
    userSpan.setTag("cloudflare.kv.operation.name"_kjc, kj::str(operationName.slice(3)));
      ...
      KJ_SWITCH_ONEOF(_options) {
        KJ_CASE_ONEOF(o2, kj::OneOf<kj::String, GetOptions>) {
          KJ_SWITCH_ONEOF(o2) {
            KJ_CASE_ONEOF(type, kj::String) {
              userSpan.setTag("cloudflare.kv.query.parameter.type"_kjc, kj::mv(type));
            }
            KJ_CASE_ONEOF(o, GetOptions) {
              ...
              KJ_IF_SOME(cacheTtl, o.cacheTtl) {
                userSpan.setTag("cloudflare.kv.query.parameter.cacheTtl"_kjc, (int64_t)cacheTtl);
              }
            }
          }
        }
        KJ_CASE_ONEOF(o, ListOptions) {
          KJ_IF_SOME(l, o.limit) {
            userSpan.setTag("cloudflare.kv.query.parameter.limit"_kjc, (int64_t)l);
          }
          KJ_IF_SOME(prefix, o.prefix) {
            KJ_IF_SOME(p, prefix) {
              userSpan.setTag("cloudflare.kv.query.parameter.prefix"_kjc, kj::mv(p));
            }
          }
          KJ_IF_SOME(cursor, o.cursor) {
            KJ_IF_SOME(c, cursor) {
              userSpan.setTag("cloudflare.kv.query.parameter.cursor"_kjc, kj::mv(c));
            }
          }
        }
        KJ_CASE_ONEOF(o, PutOptions) {
          KJ_IF_SOME(expiration, o.expiration) {
            userSpan.setTag("cloudflare.kv.query.parameter.expiration"_kjc, (int64_t)expiration);
          }
          KJ_IF_SOME(expirationTtl, o.expirationTtl) {
            userSpan.setTag(
                "cloudflare.kv.query.parameter.expirationTtl"_kjc, (int64_t)expirationTtl);
          }
        }
      }
    }
