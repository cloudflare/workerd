...
static void parseListMetadata(TraceContext& traceContext,
  ...
  js.withinHandleScope([&] {
    ...
    // jsg::JsValue listComplete = obj.get(js, LIST_COMPLETE);
    ...
    KJ_IF_SOME(_, obj.get(js, CURSOR).tryCast<jsg::JsString>()) {
    ...
    KJ_IF_SOME(expiration, obj.get(js, EXPIRATION).tryCast<jsg::JsNumber>()) {
      traceContext.userSpan.setTag(
          "cloudflare.kv.response.expiration"_kjc, int64_t(expiration.value(js).orDefault(0)));
    }
