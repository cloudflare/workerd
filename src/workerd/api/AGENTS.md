...
- **NEVER** use `JSG_INSTANCE_PROPERTY` (breaks minor GC); use `JSG_PROTOTYPE_PROPERTY`
- **NEVER** add `Fetcher` methods without compat flag (conflicts with JS RPC wildcard dispatch)
- **NEVER** change `Headers::Guard` enum numeric values (serialized on wire)
