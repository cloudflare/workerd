...
class ReplacerStreamSink final: public WritableStreamSink {
 public:
  ...
  kj::Promise<void> write(kj::ArrayPtr<const byte> buffer) override KJ_WARN_UNUSED_RESULT {
    ...
    if (err != 0) {
      return KJ_EXCEPTION(FAILED, "lol_html_streaming_sink_write_utf8_chunk failed");
    }
  ...
  void abort(kj::Exception reason) override {
    // TODO(npaun): Not sure what we can do... should we throw it again?
    // should we set a flag?
    // should we immediately poison?
    // For other errors just returning the exception will bubble it up to thunkImpl which handles poisioning for us
  }
...
int Rewriter::replacerThunkImpl(
  ...
  // TODO(npaun): This is basically the same as thunkImpl except we call replacerThunkPromise instead,
  // and return an int (because that's what lol-html says to do)
  // We could merge this with thunkImpl fairly easily if that's what we needed to do
  if (isPoisoned()) {
    // Handlers disabled due to exception.
    KJ_LOG(ERROR, "poisoned rewriter should not be able to call handlers");
    return -1;
...
BaseImpl<CType>::BaseImpl(CType& element, Rewriter& rewriter)
    : element(element),
      rewriter(rewriter) {}
template <typename CType>
BaseImpl<CType>::~BaseImpl() noexcept(false) {}
template <typename CType>
template <auto Func, auto StreamingFunc>
void BaseImpl<CType>::rewriteContentGeneric(
  ...
  KJ_SWITCH_ONEOF(content) {
    ...
    KJ_CASE_ONEOF(responseContent, jsg::Ref<Response>) {
      KJ_IF_SOME(body, responseContent->getBody()) {
        ...
        // TODO(npaun): What should we do if this happens?
      }
    }
  }
}
...
jsg::Ref<Comment> Comment::before(Content content, jsg::Optional<ContentOptions> options) {
  // TODO(npaun): Seems like you can't streaming replace comments yet in lol-html
...
jsg::Ref<DocumentEnd> DocumentEnd::append(Content content, jsg::Optional<ContentOptions> options) {
  // TODO(npaun): Seems like you can't streaming replace in lol-html yet for this type
  auto stringContent = unwrapContent(kj::mv(content));
