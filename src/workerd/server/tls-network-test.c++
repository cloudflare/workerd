...
KJ_TEST("TLS: a listener hands out a connection only once its client is authenticated") {
  auto io = kj::setupAsyncIo();
  auto& ws = io.waitScope;
  auto serverContext = makeContext([](config::TlsOptions::Builder options) {
    serveExampleCom(options);
    trust(options, CA_CERT);
    options.setRequireClientCerts(true);
  });
  ...
  auto authenticated = makeContext([](config::TlsOptions::Builder options) {
    trust(options, CA_CERT);
    auto keypair = options.initKeypair();
    keypair.setPrivateKey(CLIENT_KEY);
    keypair.setCertificateChain(CLIENT_CERT);
  ...
  auto receiver = serverContext->wrapPort(network.parseAddress("127.0.0.1", 0).wait(ws)->listen());
  auto address = network.parseAddress("127.0.0.1", receiver->getPort()).wait(ws);
  auto accepted = receiver->accept().eagerlyEvaluate(nullptr);

  {
    KJ_EXPECT_LOG(ERROR, "error accepting tls connection");
    auto refused = ping(anonymous->wrapClient(address->connect().wait(ws), "example.com")).wait(ws);
    KJ_EXPECT(refused != kj::none);
    KJ_EXPECT(!accepted.poll(ws));
  }

  auto served = echo(kj::mv(accepted)).eagerlyEvaluate(nullptr);
  KJ_EXPECT(ping(authenticated->wrapClient(address->connect().wait(ws), "example.com")).wait(ws) ==
      kj::none);
  served.wait(ws);
}
...
// A client and server pair over a socket pair, handshaken, and one byte exchanged.
struct TlsPair {
...
KJ_TEST("TLS stream: a peer closing without close_notify is a disconnect, not an EOF") {
  TlsPair p;
  kj::byte buffer[5];
  auto read = p.client->tryRead(buffer, 1, sizeof(buffer));
  p.server = nullptr;
  KJ_EXPECT_THROW(DISCONNECTED, read.wait(p.io.waitScope));
}
