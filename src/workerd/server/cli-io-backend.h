...
//  - the tokio watcher: kj-rs-io's watcher (src/rust/cxx/kj-rs-io/watcher.rs, Rust over the `notify`
//    crate: inotify / FSEvents / ReadDirectoryChangesW). It watches each file's directory and
//    judges changes by re-stamping the files, so replaced, recreated and symlinked files keep
//    firing; used under --//:io_backend=rust, where no UnixEventPort exists.
//
