...
kj::Array<byte> ChannelTokenHandler::encodeChannelTokenImpl(ChannelToken::Type type,
  ...
  switch (usage) {
    case IoChannelFactory::ChannelTokenUsage::RPC: {
      auto magic = kj::arrayPtr(ChannelToken::RPC_TOKEN_MAGIC).asBytes();

      auto result =
          kj::heapArray<byte>(magic.size() + AES_IV_SIZE + plaintext.size() + AES_MAC_SIZE);
      result.slice(0, magic.size()).copyFrom(magic);

      auto iv = result.slice(magic.size()).first(AES_IV_SIZE);
      getEntropy(iv);
      ...
      KJ_ASSERT(EVP_EncryptInit(aesCtx, EVP_aes_256_gcm(), tokenKey, iv.begin()));

      // Add magic number as AAD first.
      {
        int outSize = 0;
        KJ_ASSERT(EVP_EncryptUpdate(aesCtx, nullptr, &outSize, magic.begin(), magic.size()));
        KJ_ASSERT(outSize == 4);
      }
      ...
      {
        int outSize = 0;
        KJ_ASSERT(EVP_EncryptUpdate(aesCtx, result.begin() + magic.size() + AES_IV_SIZE, &outSize,
    ...
    case IoChannelFactory::ChannelTokenUsage::STORAGE: {
      auto magic = kj::arrayPtr(ChannelToken::STORAGE_TOKEN_MAGIC).asBytes();
...
kj::Own<Frankenvalue::CapTableEntry> ChannelTokenHandler::decodeChannelTokenImpl(
  ...
  uint32_t magic;
  KJ_REQUIRE(token.size() >= sizeof(magic));
  kj::arrayPtr(magic).asBytes().copyFrom(token.slice(0, sizeof(magic)));
  ...
  switch (usage) {
    case IoChannelFactory::ChannelTokenUsage::RPC: {
      KJ_REQUIRE(magic == ChannelToken::RPC_TOKEN_MAGIC, "invalid channel token for RPC");
      KJ_REQUIRE(token.size() >= sizeof(magic) + AES_IV_SIZE + AES_MAC_SIZE,
          "invalid channel token for RPC");

      auto iv = token.slice(sizeof(magic)).first(AES_IV_SIZE);
      auto mac = token.slice(token.size() - AES_MAC_SIZE);
      auto ciphertext = kj::arrayPtr(iv.end(), mac.begin());

      EVP_CIPHER_CTX* aesCtx = EVP_CIPHER_CTX_new();
      KJ_ASSERT(aesCtx != nullptr);
      KJ_DEFER(EVP_CIPHER_CTX_free(aesCtx));

      KJ_ASSERT(EVP_DecryptInit(aesCtx, EVP_aes_256_gcm(), tokenKey, iv.begin()));

      // Add magic number as AAD first.
      {
        int outSize = 0;
        KJ_ASSERT(EVP_DecryptUpdate(aesCtx, nullptr, &outSize, token.begin(), sizeof(magic)));
        KJ_ASSERT(outSize == 4);
      }
