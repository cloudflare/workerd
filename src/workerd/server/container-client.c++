...
class ContainerClient::DockerPort final: public rpc::Container::Port::Server {
 public:
  ...
  kj::Promise<void> connect(ConnectContext context) override {
    ...
    // httpClient must be first: Promise::attach() stores values in a kj::Tuple whose
    // elements are destroyed in reverse index order (last index first).  httpClient owns
    // the underlying proxyConnection, so it must outlive connection and downEnd.
    pumpTask =
