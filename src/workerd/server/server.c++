 ...
 public:
  ...
  kj::Own<WorkerInterface> startRequest(IoChannelFactory::SubrequestMetadata metadata,
    ...
    if (legacyTailWorkers.size() > 0 || streamingTailWorkers.size() > 0) {
      ...
      kj::Own<WorkerTracer> wT = kj::refcounted<WorkerTracer>(
          kj::none, kj::mv(trace), PipelineLogLevel::FULL, kj::mv(tailStreamWriter));

      // When the tracer is complete, deliver traces to any legacy tail workers. We end up creating
      // two references to the WorkerTracer, one held by the observer and one that will be passed to
      // the IoContext. This ensures that the tracer lives long enough to receive all events.
      if (legacyTailWorkers.size()) {
        waitUntilTasks.add(wT->onComplete().then(
