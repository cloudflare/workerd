...
struct Server::WorkerDef {
  ...
  // dynamic isolates. Same story for ContainerEngine.
...
kj::Own<Server::WorkerService> Server::makeWorkerImpl(kj::StringPtr name,
  ...
  auto linkCallback = [this, name, def = kj::mv(def)](WorkerService& workerService) mutable {
    ...
    if (workerService.hasDefaultEntrypoint()) {
      services.add(KJ_ASSERT_NONNULL(workerService.getEntrypoint(kj::none, {})));
    }
    for (auto& ep: entrypointNames) {
      services.add(KJ_ASSERT_NONNULL(workerService.getEntrypoint(ep, {})));
    }
