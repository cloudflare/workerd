  ...
  kj::Own<WorkerInterface> startRequest(IoChannelFactory::SubrequestMetadata metadata,
    ...
    if (legacyTailWorkers.size() > 0 || streamingTailWorkers.size() > 0) {
      ...
      workerTracer = tracer->makeWorkerTracer(PipelineLogLevel::FULL, executionModel,
          ...
          kj::none /* entrypoint */, kj::none);
      KJ_IF_SOME(tracer, workerTracer) {
        tracer->addTailStreamWriter(tracing::initializeTailStreamWriter(kj::mv(legacyTailWorkers),
            kj::mv(streamingTailWorkers), waitUntilTasks, tracer->getTrace()));
      }
    ...
    observer = kj::refcounted<RequestObserverWithTracer>(mapAddRef(workerTracer),
        kj::mv(legacyTailWorkers), kj::mv(streamingTailWorkers), waitUntilTasks);
