 ...
 public:
  ...
  kj::Promise<void> run() {
    ...
    for (;;) {
      ...
      // A TLS listener's accepted streams carry a TLS peer identity wrapping the network
      // identity; unwrap it via the tls-network seam (the kj::TlsPeerIdentity downcast under the
      // default build; the rustls wrapper passes the network identity through directly).
