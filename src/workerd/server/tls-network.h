...
// honor at all (bad PEM). rustls speaks TLS 1.2 and 1.3 only, so a lower minVersion allows both;
// its cipherList honors the TLS 1.2 suites it names, and a list naming none of them (keywords)
// leaves rustls' defaults. `reportConfigError` is only invoked synchronously, for non-fatal
