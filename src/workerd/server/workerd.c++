...
public:
  ...
  void enableInspector(kj::StringPtr param) {
    // The param can be either a numeric socket fd or a socket address.
    KJ_IF_MAYBE(fd, param.tryParseAs<uint>()) {
      validateSocketFd(*fd, "inspector"_kj);
      inheritedFds.add(*fd);
      server.enableInspector(
