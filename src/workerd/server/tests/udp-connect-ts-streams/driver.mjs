...
//
// Same test cases as udp-connect/, run against a config that enables the
// typescript_implemented_streams compat flag: this exercises JsReadableStream::fromPull()'s
// TypeScript arm (instead of its legacy C++ arm, proving parity
// between the two.
...
test('UDP connect() groups datagrams from one peer into a single flow (TS streams)', async () => {
  const port = await workerd.getListenPort('udp');
  const client = createSocket('udp4');
  try {
