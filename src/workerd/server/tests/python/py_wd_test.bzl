...
load("//:build/python_metadata.bzl", "BUNDLE_VERSION_INFO", "DEFAULT_PYTHON_TEST_FLAGS")
...
def _py_wd_test_helper(
    ...
    expand_template(
        ...
        substitutions = select({
            "//build/config:python_snapshots_disabled": {
    ...
    wd_test(
        ...
        args = args + select({
            "//build/config:python_snapshots_disabled": [],
            "//conditions:default": snapshot_args,
        }),
        python_snapshot_test = select({
            "//build/config:python_snapshots_disabled": False,
            "//conditions:default": make_snapshot,
        }),
        data = data,
        _configurable_data = select({
            "//build/config:python_snapshots_disabled": [],
            "//conditions:default": snapshot_data,
        }),
        load_snapshot = select({
            "//build/config:python_snapshots_disabled": None,
            "//conditions:default": load_snapshot,
        }),
...
def compute_python_flags(python_flags, skip_python_flags):
    if python_flags == "all":
        python_flags = DEFAULT_PYTHON_TEST_FLAGS
