...
#             assert i > vendor_index, f"site-packages path {path} appears before vendor path"

#     # Verify system paths come before the vendor path
#     system_paths = ["/lib/python312.zip", "/lib/python3.12", "/lib/python3.12/lib-dynload"]
#     for path in system_paths:
#         if path in sys.path:
#             assert sys.path.index(path) < vendor_index, f"System path {path} appears after vendor path"
