...
class TmpDirectory final: public Directory {
 public:
  ...
  bool remove(
      jsg::Lock& js, kj::PathPtr path, kj::Maybe<RemoveOptions> options = kj::none) override {
    KJ_IF_SOME(dir, tryGetDirectory()) {
      return dir->remove(js, path, options);
    }
    return false;
  }
...
class LazyDirectory final: public Directory {
 public:
  ...
  bool remove(
      jsg::Lock& js, kj::PathPtr path, kj::Maybe<RemoveOptions> options = kj::none) override {
    return getDirectory()->remove(js, path, options);
  }
