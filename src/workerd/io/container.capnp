...
interface Container @0x9aaceefc06523bca {
  ...
  # the container engine should implicitly kill the container.
  ...
  # the client invoked `kill()`, then `monitor()` completes without an error. If it shuts down for
  # any other reason, `monitor()` throws an exception describing what happened. (This exception may
  # or may not be a JSG exception depending on whether it is an application error or a system
  # error.)

  kill @3 ();
  # Immediately and abruptly stops the container and tears it down. The application is not given
  # any warning, it simply stops immediately. Upon successful return from kill(), the container is
  # no longer running. If a call to `monitor()` is waiting when `kill()` is invoked, `monitor()`
  # will also return (with no error). If the container is not running when `kill()` is invoked,
  # `kill()` silently returns with no error.
