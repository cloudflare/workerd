...
// path (addExceptionToTrace) and the console.* logging path (handleLog), so that an error
...
// The console methods wrapped by setupContext (debug, error, info, log, warn).
struct ConsoleMethod {
  const char* name;
  v8::FunctionCallback callback;
};

// The C++ decorator template for console.* methods, parameterized by the embedder-data slot
// holding the method's saved original and by the method's log level.
...
constexpr ConsoleMethod kConsoleMethods[] = {
  {"debug", &consoleDecorator<jsg::ContextDataSlot::CONSOLE_ORIGINAL_DEBUG, LogLevel::DEBUG_>},
  {"error", &consoleDecorator<jsg::ContextDataSlot::CONSOLE_ORIGINAL_ERROR, LogLevel::ERROR>},
  {"info", &consoleDecorator<jsg::ContextDataSlot::CONSOLE_ORIGINAL_INFO, LogLevel::INFO>},
  {"log", &consoleDecorator<jsg::ContextDataSlot::CONSOLE_ORIGINAL_LOG, LogLevel::LOG>},
  {"warn", &consoleDecorator<jsg::ContextDataSlot::CONSOLE_ORIGINAL_WARN, LogLevel::WARN>},
};

constexpr size_t kConsoleMethodsCount = kj::size(kConsoleMethods);

// setupContext saves each method's original into the slot CONSOLE_ORIGINAL_DEBUG + i, so the
// table must hold exactly one entry per CONSOLE_ORIGINAL_* slot, in the same order.
...
void Worker::setupContext(jsg::Lock& lock, v8::Local<v8::Context> context) {
  ...
  auto consoleStr = lock.strIntern("console");
  ...
  for (size_t i = 0; i < kConsoleMethodsCount; ++i) {
    auto methodStr = lock.strIntern(kConsoleMethods[i].name);
    auto original = jsg::check(console->Get(context, methodStr)).As<v8::Function>();
    jsg::setContextDataSlot(context, jsg::consoleOriginalSlot(i), original);
