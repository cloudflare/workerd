...
interface EventDispatcher @0xf20697475ec1752d {
  ...
  restoreService @12 (params :Frankenvalue) -> (service :EventDispatcher);
  restoreRpcStub @13 (params :Frankenvalue) -> (target :JsRpcTarget);
  ...
  # that.
  #
  # Note that the `EventDispatcher` returned by `restoreService`, as always, can only be used for
  # a single call. However, the `JsRpcTarget` returned by `restoreRpcStub` can be used for any
  # number of calls, with the caveat that it may become disconnected at some point, requiring
  # re-restoring it.
  #
