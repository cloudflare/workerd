...
class ActorCacheInterface: public ActorCacheOps {
 public:
  ...
  virtual kj::OneOf<CancelAlarmHandler, RunAlarmHandler> armAlarmHandler(
      kj::Date scheduledTime, bool noCache, kj::StringPtr actorId, SpanParent parentSpan) = 0;
...
class ActorCache::Transaction final: public ActorCacheInterface::Transaction {
 ...
 private:
  ...
  // Trace span captured from the first write, to be used when commit() flushes changes.
  SpanParent commitSpan = nullptr;
  bool hasCommitSpan = false;
