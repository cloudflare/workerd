...
void WorkerTracer::setFetchResponseInfo(tracing::FetchResponseInfo&& info) {
  ...
  KJ_LOG(ERROR, "setFetchResponseInfo", topLevelInvocationSpanContext != kj::none);
    auto& spanContext = KJ_UNWRAP_OR_RETURN(topLevelInvocationSpanContext);
     writer->report(spanContext, tracing::Return(tracing::Return::Info(KJ_ASSERT_NONNULL(trace->fetchResponseInfo).clone())));
  }
}
