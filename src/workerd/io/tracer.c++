...
void PipelineTracer::addTracesFromChild(kj::ArrayPtr<kj::Own<Trace>> traces, bool isLevel) {
  // TODO(now): Managing order in which traces are submitted to support tests.
  if (isLevel) {
...
kj::Own<WorkerTracer> PipelineTracer::makeWorkerTracer(PipelineLogLevel pipelineLogLevel,
  ...
  // TODO: In new model, we do not keep track of trace generated by WorkerTracer directly. Can clean
  // up lots of code here.
...
void WorkerTracer::addLog(const tracing::InvocationSpanContext& context,
  ...
  // Still send logs that are too large, backend handles truncation.
...
void WorkerTracer::addException(const tracing::InvocationSpanContext& context,
  ...
  // Still send exceptions that are too large, backend handles truncation.
  /*if (newSize > MAX_TRACE_BYTES) {
...
void WorkerTracer::setOutcome(EventOutcome outcome, kj::Duration cpuTime, kj::Duration wallTime) {
  ...
  KJ_IF_SOME(writer, maybeTailStreamWriter) {
    ...
    // TODO: This needs to be removed to propagate time to LTW reliably.
