...
kj::Maybe<kj::Own<IoContext::WeakRef>>& BaseTracer::getWeakIoContext() {
  return weakIoContext;
}
kj::Date BaseTracer::getCompleteTime() {
  return completeTime;
}
...
kj::Maybe<kj::Date> UserSpanObserver::getTime() {
  if (isPredictableModeForTest()) {
    return kj::UNIX_EPOCH;
  }

  return submitter->getTime();
}
