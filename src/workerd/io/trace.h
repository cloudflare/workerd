...
inline kj::Maybe<tracing::SpanId> SpanBuilder::getSpanId() {
  return observer.map([](kj::Own<SpanObserver>& obs) {
...
class TraceContext {
 public:
  ...
  // Add a timestamped log entry to the internal span
  // User-facing log attribution is handled via InvocationSpanContext/tracer events.
