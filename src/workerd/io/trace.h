...
class SpanObserver: public kj::Refcounted {
 public:
  ...
  // isn't used directly to create a span before the IoContext has been constructed, and that spans
  // spans are not owned by structures that outlive the IOContext (previously this was a case with a
  // top-level span owned by the WorkerTracer itself).
  // Note that we do not attempt to be clever and gather I/O time directly in SpanBuilder to avoid
  // having trace depend on io-context.
  // TODO(o11y): Add a test for this actually being I/O time by confirming that time stamps don't
  // progress much when spinning for some time.
