...
//
// TODO(soon): Introduce a codec which can minimize the number of flags generated by either variant
//   below by choosing a good compatibility date.
kj::Array<kj::StringPtr> decompileCompatibilityFlagsForFl(CompatibilityFlags::Reader input);

// Like decompileCompatibilityFlagsForFl, but returns EVERY set flag, not just those annotated
// `$neededByFl`. Use when propagating a worker's full effective compatibility to another component.
