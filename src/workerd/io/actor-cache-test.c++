...
KJ_TEST("ActorCache alarm get/put") {
...

  {
    ...
    auto armResult =
        test.cache.armAlarmHandler(10 * kj::SECONDS + kj::UNIX_EPOCH, testCurrentTime, false);
...

  {
    ...
    {
      auto armResult = test.cache.armAlarmHandler(oneMs, testCurrentTime, false);
...

  {
    ...
    {
      auto armResult = test.cache.armAlarmHandler(twoMs, testCurrentTime, true);
      KJ_ASSERT(armResult.is<ActorCache::RunAlarmHandler>());
    }
    mockStorage->expectCall("deleteAlarm", ws)
...
KJ_TEST("ActorCache alarm delete when flush fails") {
  ...
  {
    auto handle = test.cache.armAlarmHandler(oneMs, testCurrentTime, false);
