...
KJ_TEST("AbortableInputStream tryReadSync") {
  ...
  auto pipe = kj::newOneWayPipe();
  auto canceler = kj::refcounted<RefcountedCanceler>();
  auto stream = kj::refcounted<AbortableInputStream>(kj::mv(pipe.in), *canceler);
  ...
  canceler->cancel(KJ_EXCEPTION(DISCONNECTED, "aborted"));
  KJ_EXPECT(stream->tryReadSync(kj::arrayPtr(buf), 1) == kj::none);
}
