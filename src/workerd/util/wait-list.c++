...
XThreadWaitList<void>::XThreadWaitList(Options options)
    : state(kj::atomicRefcounted<State>(options)) {}

void XThreadWaitList<void>::destroyed() {
  if (!createdFulfiller) state->lostFulfiller();
}

XThreadWaitList<void>::Waiter::Waiter(
...
XThreadWaitList<void>::Waiter::~Waiter() noexcept(false) {
...
void XThreadWaitList<void>::Waiter::Registration::ready(const Outcome& outcome) noexcept {
  KJ_IF_SOME(exception, outcome.exception) {
    waiter.fulfiller->reject(exception.clone());
  } else {
...
void XThreadWaitList<void>::Waiter::Registration::removed() noexcept {
  waiter.fulfiller->reject(makeNeverFulfilledException());
  __atomic_store_n(&waiter.unlinked, true, __ATOMIC_RELEASE);
}

kj::Promise<void> XThreadWaitList<void>::addWaiter() const {
  ...
  if (state->useThreadLocalOptimization) {
    ...
    auto& weak = threadLocalWaiters->findOrCreate(
        state.get(), [&]() -> XThreadWaitList<void>::WaiterMap::Entry {
...
kj::Own<kj::CrossThreadPromiseFulfiller<void>> XThreadWaitList<void>::makeSeparateFulfiller() {
...
bool XThreadWaitList<void>::State::tryFulfill() const {
  return list.tryReady(Outcome{kj::none});
}

void XThreadWaitList<void>::State::reject(kj::Exception&& e) const {
