...
// kj's HTTP entry points as workerd calls them -- kj::HttpServer, kj::newHttpClient(),
// kj::newWebSocketPipe() -- checked against the wire and against kj's contracts. The same tests
...
KJ_TEST("WebSocketPipe send completes only once the message is received") {
  auto io = kj::setupAsyncIo();
  auto pipe = kj::newWebSocketPipe();
  auto send = pipe.ends[0]->send("a"_kj).eagerlyEvaluate(nullptr);
  KJ_EXPECT(!send.poll(io.waitScope));
  KJ_EXPECT(pipe.ends[1]->receive().wait(io.waitScope).get<kj::String>() == "a");
  send.wait(io.waitScope);
}

KJ_TEST("WebSocketPipe never delivers a cancelled send") {
  auto io = kj::setupAsyncIo();
  auto pipe = kj::newWebSocketPipe();
  {
    auto lost = pipe.ends[0]->send("lost"_kj).eagerlyEvaluate(nullptr);
    KJ_EXPECT(!lost.poll(io.waitScope));
  }
  auto kept = pipe.ends[0]->send("kept"_kj).eagerlyEvaluate(nullptr);
  KJ_EXPECT(pipe.ends[1]->receive().wait(io.waitScope).get<kj::String>() == "kept");
  kept.wait(io.waitScope);
}

class FailingWebSocket final: public kj::WebSocket {
 public:
  ...
  kj::Promise<Message> receive(size_t maxSize) override {
    return KJ_EXCEPTION(FAILED, "the pump's source failed");
  }
...
KJ_TEST("a pump into a WebSocketPipe hands its failure to the reader") {
  auto io = kj::setupAsyncIo();
  auto pipe = kj::newWebSocketPipe();
  FailingWebSocket source;
  auto pump = source.pumpTo(*pipe.ends[0]).catch_([](kj::Exception&&) {}).eagerlyEvaluate(nullptr);
  KJ_EXPECT_THROW_MESSAGE("the pump's source failed", pipe.ends[1]->receive().wait(io.waitScope));
}

// =======================================================================================
// Recovered from kj-hyper's former C++ suites, rewritten against kj's interfaces so they hold
// both backends to kj's behavior.
