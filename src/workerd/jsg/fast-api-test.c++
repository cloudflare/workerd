...
class FastMethodContext: public jsg::Object, public jsg::ContextGlobal {
 public:
  ...
  bool complexMethod(jsg::Promise<bool> promise1, kj::String another, jsg::Promise<bool> promise, kj::String oneByteString) {
    KJ_ASSERT(another == "cloudflare");
      KJ_ASSERT(oneByteString == "erik");
    return true;
  }

  v8::Local<v8::String> getErik(jsg::Lock& js) {
    kj::String data = kj::str("erik");
    return jsg::check(v8::String::NewFromOneByte(js.v8Isolate, data.asBytes().begin()));
  }
...
jsg::V8System v8System({"--allow-natives-syntax", "--expose-gc"});
...
CallCounter runTest(Test test) {
  ...
  e.expectEval(kj::str("const getErik2 = () => { return getErik(); }; const fastCall = () => { return ", target,
                   "; }; "
                   "%PrepareFunctionForOptimization(fastCall); "
                   "fastCall(); "
                   "%OptimizeFunctionOnNextCall(fastCall); "
                   "let garbage = 'a'.repeat(Math.floor(Math.random() * 100)); garbage = null;"
                   "fastCall()"),
      test.expectedReturnType, test.expectedReturnValue);
  return jsg::callCounter;
}

KJ_TEST("Trigger GC causes issues") {
  util::Autogate::initAutogateNamesForTest({"v8-fast-api"_kj});

  KJ_ASSERT(runTest({"complexMethod(Promise.resolve(false), \"cloudflare\", Promise.resolve(true), getErik2())"_kjc, "boolean"_kjc,
              "true"_kjc}) == CallCounter(3, 1));
}
