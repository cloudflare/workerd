...
// names every API type registered with the isolate, which is ~14kB of text when spelled out.
// That spelling ends up in the mangled name -- and the DWARF linkage name -- of every one of its
// member instantiations, and the members below have thousands of instantiations between them, so
// leaving them in `TypeWrapper` costs tens of megabytes of symbol names and debug info. Hoisting
// them into a base parameterized only on the short-named most-derived type keeps those names
// small. See also the comment on `TypeWrapper::from()`.
//
