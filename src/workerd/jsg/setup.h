...
class V8System {
 public:
  ...
  typedef void FatalErrorCallback(kj::StringPtr location, kj::StringPtr messagex);
...
class IsolateBase {
 ...
 private:
  ...
  kj::MutexGuarded<kj::HashSet<ExternalMemoryPtr*>> externalMemoryPointers;

  // Remove an external memory pointer
  void removeExternalMemoryPointer(ExternalMemoryPtr* externalMemoryPtr);
  ...
  friend class ExternalMemoryPtr;
