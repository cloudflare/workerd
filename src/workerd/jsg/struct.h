...
//
// The struct that declares the field, the field's location within that struct, and the field's
// JS-visible name are all supplied by StructWrapper at the call site rather than baked into the
// template arguments. As a result this class is instantiated once per field *type*, not once per
// field: a JSG_STRUCT with ten `Optional<kj::String>` fields shares one instantiation, as do all
// the `Optional<kj::String>` fields of every other struct in the binary.
