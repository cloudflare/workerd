...
// Converts one field of a JSG_STRUCT to/from the corresponding JavaScript property. Note that a
// field wrapper's state is just its interned JS-visible name – the struct that declares the field,
// the field's location within that struct and the field's type are supplied by StructWrapper at the
// call site. This avoids excessive templating, only wrap() and unwrap() need to be emitted once per
// field type.
class FieldWrapper {
 public:
...
class StructWrapper<Self, T, StructFields<fields...>, kj::_::Indexes<indices...>> {
 ...
 private:
  ...
  // One wrapper per field, in declaration order, so that fieldWrappers lines up with the fields
  // pack and with T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY.
  ...
  kj::ArrayPtr<FieldWrapper> getFields(v8::Isolate* isolate) {
    KJ_IF_SOME(f, lazyFields) {
      ...
      auto builder = kj::heapArrayBuilder<FieldWrapper>(
          kj::size(T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY));
      for (auto name: T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY) {
  ...
  static v8::Local<v8::DictionaryTemplate> makeTemplate(v8::Isolate* isolate) {
    ...
    for (auto i: kj::indices(T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY)) {
      if (isUsable[i]) {
        names.add(std::string_view(T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY[i].begin(),
            T::_JSG_STRUCT_FIELD_NAMES_DO_NOT_USE_DIRECTLY[i].size()));
      }
    }
