...
// Converts one field of a JSG_STRUCT to/from the corresponding JavaScript property.
//
// A field wrapper's state is just its interned JS-visible name; the struct that declares the
// field, the field's location within that struct and the field's type are all supplied by
// StructWrapper at the call site. That makes all field wrappers the same type, so a struct can
// hold them in an array, and it makes wrap() and unwrap() the only code emitted per field -- once
// per field type, shared by every field of that type in the binary.
class FieldWrapper {
 public:
  ...
  void wrap(Lock& js,
    ...
    if constexpr (kj::isSameType<T, SelfRef>()) {
      ...
      if constexpr (webidl::OptionalType<T>) {
  ...
  // `structType` identifies the struct declaring this field; it appears in type error messages.
  ...
  T unwrap(TypeWrapper& wrapper,
    ...
    return wrapper.template unwrap<T>(
...
class StructWrapper<Self, T, StructFields<fields...>, kj::_::Indexes<indices...>> {
 ...
 private:
  ...
  // One wrapper per field, in declaration order, so that `fieldWrappers[i]` lines up with the i'th
  // element of the `fields` pack and with `T::jsgFieldNames[i]`.
  kj::Maybe<kj::Array<FieldWrapper>> lazyFields;

  kj::ArrayPtr<FieldWrapper> getFields(v8::Isolate* isolate) {
    KJ_IF_SOME(f, lazyFields) {
      return f;
    } else {
      auto builder = kj::heapArrayBuilder<FieldWrapper>(kj::size(T::jsgFieldNames));
      for (auto name: T::jsgFieldNames) {
        builder.add(isolate, name);
      }
      return lazyFields.emplace(builder.finish());
    }
  }
