...
jsg::ByteString JsDate::toUTCString(jsg::Lock& js) const {
  return jsg::ByteString(JsString(inner->ToUTCString()).toString(js));
}
...
JsDate Lock::date(kj::StringPtr date) {
  auto result = jsg::check(v8::Date::Parse(v8Context(), str(date)));
  JSG_REQUIRE(result->IsDate(), TypeError, "Date.parse did not return a Date");
  return JsDate(result.As<v8::Date>());
}
