...
//     - tryConsumeResolved()    - Get value if already resolved (optimization)
//
//   Conversion:
...
//     kj::Maybe<DeferredPromise<Resource>> pending;
//
//    public:
...
//         // Initialization in progress - piggyback on it
//         // Use whenResolved() since multiple callers may be waiting
//         return p.whenResolved(js).then(js, [this](Lock& js) {
//           return KJ_ASSERT_NONNULL(cached).clone();
//         });
//       }
//
//       // Start initialization
//       auto [promise, resolver] = newDeferredPromiseAndResolver<Resource>();
//       pending = promise.addRef();  // Keep a reference for other callers
//
//       initializeAsync().then([this, resolver = kj::mv(resolver)](Resource r) {
//         cached = kj::mv(r);
//         pending = kj::none;
//         resolver.resolve(js, cached->clone());
//       });
//
//       return kj::mv(promise);
//     }
...
class ContinuationQueue {
 public:
  ...
  // If not, we start draining (which will run this and any subsequently queued
  // continuations).
  void schedule(Lock& js, kj::Function<void(Lock&)> continuation) {
 ...
 private:
  void drain(Lock& js) {
    ...
    // Process queue in FIFO order for predictable execution
    // We use an index rather than removing from front to avoid O(n^2) behavior
    while (drainIndex < queue.size()) {
      auto continuation = kj::mv(queue[drainIndex]);
      ++drainIndex;
      continuation(js);
    }
...
// Thread-local continuation queue. Safe because DeferredPromise must be used
// on a single thread (the one owning the jsg::Lock).
...
struct ThenCallbackType {
  using Type = kj::Function<void(Lock&, T)>;
};

template <>
struct ThenCallbackType<void> {
  using Type = kj::Function<void(Lock&)>;
};
...
using CatchCallback = kj::Function<void(Lock&, Value)>;
...
class DeferredPromiseState final: public kj::Refcounted {
 public:
  ...
  struct Pending {
    ...
    kj::Vector<kj::Function<void(Lock&, bool /* succeeded */)>> resolutionObservers;
  ...
  void resolve(Lock& js, T value) {
    KJ_IF_SOME(pending, state.template tryGetUnsafe<Pending>()) {
      ...
      // Notify JS resolver if one exists
      ...
      KJ_IF_SOME(cb, callback) {
        auto& resolved = state.template getUnsafe<Resolved>();
        auto val = kj::mv(resolved.value);
        getContinuationQueue().schedule(
            js, [c = kj::mv(cb), v = kj::mv(val)](Lock& js) mutable { c(js, kj::mv(v)); });
      }
    }
  }
  ...
  void reject(Lock& js, Value error) {
    KJ_IF_SOME(pending, state.template tryGetUnsafe<Pending>()) {
      ...
      KJ_IF_SOME(cb, callback) {
        auto& rejected = state.template getUnsafe<Rejected>();
        auto err = kj::mv(rejected.error);
        getContinuationQueue().schedule(
            js, [c = kj::mv(cb), e = kj::mv(err)](Lock& js) mutable { c(js, kj::mv(e)); });
      }
    }
  }
 ...
 public:
  ...
  void resolve(Lock& js) {
    KJ_IF_SOME(pending, state.template tryGetUnsafe<Pending>()) {
      ...
      KJ_IF_SOME(resolver, pending.jsResolver) {
        resolver.resolve(js);
        state.template transitionTo<Resolved>();
        return;
      }
...
class DeferredPromise {
 public:
  ...
  static DeferredPromise resolved(U&& value) {
    ...
    state->state.template transitionTo<typename _::DeferredPromiseState<T>::Resolved>(
        kj::mv(value));
    return DeferredPromise(kj::mv(state));
  }
  ...
  static DeferredPromise resolved()
    requires VoidType<T>
  {
    auto state = kj::rc<_::DeferredPromiseState<void>>();
    state->state.template transitionTo<typename _::DeferredPromiseState<void>::Resolved>();
    return DeferredPromise(kj::mv(state));
  }
  ...
  static DeferredPromise rejected(Lock& js, v8::Local<v8::Value> exception) {
    auto state = kj::rc<_::DeferredPromiseState<T>>();
    state->state.template transitionTo<typename _::DeferredPromiseState<T>::Rejected>(
        Value(js.v8Isolate, exception));
    return DeferredPromise(kj::mv(state));
  }
  ...
  static DeferredPromise fromJsPromise(Lock& js, Promise<T>&& promise) {
    ...
    // Attach continuations to the jsg::Promise that resolve/reject the DeferredPromise
  ...
  // Optimization: Get the resolved value if already resolved, without
  // consuming the promise. Returns kj::none if pending or rejected.
 ...
 private:
  ...
  DeferredPromise<Output> thenImpl(Lock& js, Func&& func, ErrorFunc&& errorFunc) {
    ...
    // Create the result promise's shared state
