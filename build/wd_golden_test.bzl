"""
Macro for golden output tests that verify stdout/stderr against expected files.
"""
...
def wd_golden_test(
        ...
        args = [],
        **kwargs):
    """Rule to define tests that verify workerd output against golden files.
    ...
    sh_test(
        name = test_name,
        srcs = ["//:build/test_golden_output.sh"],
        args = test_args,
        data = test_data,
        **kwargs
    )
