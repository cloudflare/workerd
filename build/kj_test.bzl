...
def kj_test(
    ...
    cc_binary(
        ...
        deps = [
            ...
            # The tokio kj::setupAsyncIo() is linked per BINARY: every test binary links it here, and
            # no library carries it, so a binary can never end up with two event-loop
            # implementations in its link.
