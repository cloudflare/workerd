...
MODULE_TEMPLATE = """    (name = "{name}", src = embed "{path}", {ts_declaration}type = {type})"""
...
def _gen_api_bundle_capnpn_impl(ctx):
    ...
    modules += [
        _render_module(ctx.attr.bootstrap_modules[m], m.label, "bootstrap")
        for m in ctx.attr.bootstrap_modules
    ]
...
gen_api_bundle_capnpn = rule(
    ...
    attrs = {
        ...
        "bootstrap_modules": attr.label_keyed_string_dict(allow_files = True),
