...
The gate //src/workerd/server:rust-io-hermeticity is an ordinary (non-`manual`) target, so every
wildcard build and test run -- `bazel build //...`, every CI lane -- analyzes it; in the rust
config a violation fails that build.
