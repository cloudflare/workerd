...
DEPS_BAZEL_TOP = """# WARNING: THIS FILE IS AUTOGENERATED BY update-deps.py DO NOT EDIT

"""

# Global dict to collect metadata for all repositories
ALL_REPO_METADATA = {}
...
def gen_repo_bzl(repo, current_dep, current_metadata):
    ...
    if TARGET_FILTER is not None and not repo["name"].startswith(TARGET_FILTER):
        print("skipped")
        # Preserve existing metadata for skipped repos
        if repo["name"] in current_metadata:
            ALL_REPO_METADATA[repo["name"]] = current_metadata[repo["name"]]
        return current_dep
...
def process_deps(deps, current_deps, current_metadata, deps_bzl):
    ...
    for repo in sorted_repos:
        content = gen_repo_bzl(repo, current_deps.get(repo["name"]), current_metadata)
...
def process_config(deps_file, current_metadata):
    ...
    try:
        with deps_path.open() as fp:
            ...
            process_deps(
                json.loads(json_text), current_deps, current_metadata, bzl_path
            )
    except FileNotFoundError:
        pass


def to_starlark_name(name):
    """Convert a repository name to a valid Starlark variable name (uppercase with underscores)."""
    return name.replace("-", "_").replace(".", "_").upper()


def parse_deps_bazel(file: Path) -> dict[str, dict]:
    """Parse existing deps.bazel file to extract metadata for each repository."""
    try:
        text = file.read_text()
    except FileNotFoundError:
        return {}

    # deps.bazel is Python-compatible, just exec it and extract MODULES
    namespace = {}
    exec(text, namespace)
    return namespace.get("MODULES", {})


def gen_deps_bazel_metadata():
    """Generate deps.bazel file containing metadata for all repositories."""
    deps_bazel_path = GEN_DIR / "deps.bazel"

    content = DEPS_BAZEL_TOP

    # Sort repositories by name for consistent output
    sorted_names = sorted(ALL_REPO_METADATA.keys())

    # Generate individual repository constants
    for name in sorted_names:
        metadata = ALL_REPO_METADATA[name]
        starlark_name = to_starlark_name(name)
        content += f"{starlark_name} = {pprint.pformat(metadata, sort_dicts=True)}\n\n"

    # Generate the MODULES dict mapping original names to constants
    content += "MODULES = {\n"
    for name in sorted_names:
        starlark_name = to_starlark_name(name)
        content += f'    "{name}": {starlark_name},\n'
    content += "}\n"

    with deps_bazel_path.open("w") as f:
        f.write(content)
...
def run():
    ...
    if TARGET_FILTER is None:
        ...
        # Load existing deps.bazel metadata for incremental updates
        deps_bazel_path = GEN_DIR / "deps.bazel"
        current_metadata = parse_deps_bazel(deps_bazel_path)

    for deps in ALL_DEPS:
        process_config(deps, current_metadata)

    gen_deps_bazel_metadata()
