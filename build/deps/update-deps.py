...
ALL_DEPS = {
    SCRIPT_DIR / "deps.jsonc": GEN_DIR / "deps.bzl",
    SCRIPT_DIR / "build_deps.jsonc": GEN_DIR / "build_deps.bzl",
    SCRIPT_DIR / "shared_deps.jsonc": GEN_DIR / "shared_deps.bzl",
}
...
def gen_github_release(repo):
    ...
    if repo.get("use_tarball") or len(assets) == 0:
        if "tarball_url" in release and "file_regex" not in repo:
...
def process_config(deps_path, bzl_path):
    with deps_path.open() as fp:
        json_text = strip_comments(fp.read())
        process_deps(json.loads(json_text), bzl_path)
...
def run():
    ...
    for deps, bzl in ALL_DEPS.items():
        process_config(deps, bzl)
