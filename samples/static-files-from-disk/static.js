...
export default {
  async fetch(req, env) {
    ...
    let content = await env.files.fetch('http://dummy' + path, {
      method: req.method,
    });
    ...
    if (content.status == 404) {
      if (config.hideHtmlExtension && !path.endsWith('.html')) {
        ...
        path = path + '.html';
        content = await env.files.fetch('http://dummy' + path, {
          method: req.method,
        });
      }
      ...
      if (!content.ok) {
        ...
        //
        // Look for a 404 page.
        content = await env.files.fetch('http://dummy/404.html', {
          method: req.method,
        });
    ...
    if (!content.ok) {
      ...
      console.error(
        "Fetching '" + path + "' returned unexpected status: " + content.status
      );
    ...
    if (content.headers.get('Content-Type') == 'application/json') {
      ...
      if (path.endsWith('/')) {
        ...
        if (req.method != 'HEAD') {
          let html = await makeListingHtml(
            origPath,
            await content.json(),
            env.files
          );
...
async function makeListingHtml(path, listing, dir) {
  ...
  for (let file of listing) {
    ...
    if (file.type == 'file' || file.type == 'directory') {
      let meta = await dir.fetch('http://dummy' + path + file.name, {
        method: 'HEAD',
      });
      console.log(
        meta.status,
        'http://dummy' + path + file.name,
