...
def check_clang_format() -> bool:
    try:
        ...
        output = subprocess.check_output(["clang-format", "--version"]).decode("utf-8")
...
def find_cpp_files(dir_path) -> typing.List[str]:
    files = []
...
def clang_format(files: typing.List[str], check=False):
    if not files:
        logging.info("No changes to format")
        exit(0)
    if check:
        result = subprocess.run(
            ["clang-format", "--dry-run", "--Werror"] + files, check=False
        )
        if result.returncode != 0:
            logging.error("Code has lint. Fix with: make format")
            exit(1)
    else:
...
def git_get_modified_files(
    target: str, source: typing.Optional[str], staged: bool
) -> typing.List[str]:
    if staged:
        files_in_diff = (
