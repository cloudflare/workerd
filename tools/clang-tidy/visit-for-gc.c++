...
bool endsWithQualified(llvm::StringRef qualifiedName, llvm::StringRef suffix) {
...
std::string getTemplateQualifiedName(clang::QualType qt) {
...
bool isVisitableType(clang::QualType qt) {
...
bool isInJsgNamespace(const clang::Decl *decl) {
...
void VisitForGcCheck::registerMatchers(clang::ast_matchers::MatchFinder *Finder) {
...
void VisitForGcCheck::onStartOfTranslationUnit() {
  records_.clear();
  usedAsField_.clear();
...
void VisitForGcCheck::check(const clang::ast_matchers::MatchFinder::MatchResult &Result) {
...
void VisitForGcCheck::onEndOfTranslationUnit() {
...
void VisitForGcCheck::recordUsedAsField(clang::QualType qt) {
  if (qt.isNull()) return;
  qt = qt.getNonReferenceType().getUnqualifiedType();
...
void VisitForGcCheck::checkRecord(const clang::CXXRecordDecl *record) {
...
bool VisitForGcCheck::baseHasVisitForGc(const clang::CXXRecordDecl *record) {
