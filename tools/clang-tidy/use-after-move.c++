...
bool isDirectDerivedFieldUse(const clang::DeclRefExpr& expression,
  ...
  while (true) {
    ...
    if (parent == nullptr ||
        !clang::isa<clang::ImplicitCastExpr, clang::ParenExpr>(parent)) {
      return false;
    }
    node = parents[0];
  }
...
bool onlyUsesDirectDerivedFieldsAfterBaseMove(
  ...
  for (const auto* initializer: constructor->inits()) {
    if (!afterMove && initializer->getInit()->IgnoreImplicit() == movingInitializer->IgnoreImplicit()) {
      afterMove = true;
      continue;
    }
    if (afterMove && hasNonDerivedFieldUse(*initializer->getInit(), *argument.getDecl(),
