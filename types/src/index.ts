...
export async function main(args?: string[]) {
  ...
  const maybeCapnpPath = options["input-capnp"];
  const maybeAstJsonPath = options["input-ast-json"];

  if (maybeAstJsonPath === undefined) {
    console.warn(`No AST passed in, parameter names will be autogenerated.`);
  } else {
    parseApiAstDump(maybeAstJsonPath);
  }

  const buffer =
    maybeCapnpPath === undefined
      ? await arrayBuffer(process.stdin)
      : await readFile(maybeCapnpPath);
