...
export async function main(args?: string[]) {
  const { values: options } = util.parseArgs({
    options: {
      defines: { type: "string", short: "d" },
      output: { type: "string", short: "o" },
      format: { type: "boolean", short: "f" },
      "input-capnp": { type: "string", multiple: true },
      "input-ast-json": { type: "string" },
    },
    strict: true,
  ...
  assert(
    options["input-capnp"],
    "This script requires at least one --input-capnp arg pointing to a binary Cap’n Proto file"
  );

  const extra = await collateExtraDefinitions(options.defines);

  const files = await readdir(inputDir);
  if (options["input-ast-json"] === undefined) {
    console.warn(`No AST passed in, parameter names will be autogenerated.`);
  } else {
    parseApiAstDump(options["input-ast-json"]);
  }
  ...
  for (const file of options["input-capnp"]) {
