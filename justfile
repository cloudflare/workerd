...
wpt-test test_name:
  just stream-test //src/workerd/api/wpt:{{test_name}}

lldb-wpt-test test_name: build
  cd bazel-bin/src/workerd/api/wpt/{{test_name}}.runfiles/workerd/src/workerd/api/wpt; lldb ../../../workerd/server/workerd  -- test {{test_name}}.wd-test --experimental --directory-path=TEST_TMPDIR=/tmp

asan-wpt-test test_name:
  bazel test //src/workerd/api/wpt:{{test_name}} --config=asan --config=debug
